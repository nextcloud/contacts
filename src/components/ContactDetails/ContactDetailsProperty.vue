  ...
  computed: {
    ...
    componentInstance() {
      ...
      if (this.property.isMultiValue && this.propType === 'text') {
        ...
        this.propType &&
        ['date-and-or-time', 'date-time', 'time', 'date'].indexOf(
          this.propType,
        ) > -1
      ) {
    ...
    propLabel() {
      ...
      const dottedLabel = this.localContact.vCard.getFirstProperty(
        `${group}.x-ablabel`,
      );
    ...
    selectType: {
      ...
      set(data) {
        ...
        if (this.propLabel && data.id === this.propLabel.name) {
          ...
          const paramGroups = this.localContact.jCal[1].filter(
            (prop) => prop[1] && prop[1].group === group,
          );
          const allGroups = [
    ...
    value: {
      get() {
        ...
        if (this.propName === 'x-managersname') {
          ...
          const other = this.otherContacts(this.contact).find(
            (contact) => contact.displayName === displayName,
          );
          return other?.key;
        }
