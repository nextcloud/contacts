	...
	computed: {
		...
		usedProperties() {
			...
			return [...new Set(allKeys)].filter(key => {
				const valA = this.dividedProperties[0][key] ? this.getPropertyValue(this.dividedProperties[0][key]) : null
				const valB = this.dividedProperties[1][key] ? this.getPropertyValue(this.dividedProperties[1][key]) : null
				return (
		...
		conflictInformation() {
			...
			this.usedProperties.forEach(property => {
				if (this.checkIfPropertyEmpty(this.dividedProperties[0][property])) {
				...
				if (this.checkIfPropertyEmpty(this.dividedProperties[1][property])) {
					conflictInformation[property] = {
						type: 'onlyInFirst',
	...
	methods: {
		...
		async mergeContacts() {
			...
			this.usedProperties.forEach(name => {
				if (finalProperties[name] !== undefined) {
					finalProperties[name].forEach((property, index) => {
						if (index === 0) {
