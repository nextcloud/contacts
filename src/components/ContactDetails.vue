...
export default defineComponent({
  ...
  computed: {
    ...
    emailAddressProperties() {
      return this.localContact.properties.find(
  ...
  methods: {
        ...
        if (contact.dav) {
          try {
            ...
            if (error.name === 'ParserError') {
              showError(
                t('contacts', 'Syntax error. Cannot open the contact.'),
              );
    ...
    async moveContactToAddressbook(addressbookId) {
      const addressbook = this.addressbooks.find(
        (search) => search.id === addressbookId,
      );
      this.loadingUpdate = true;
      if (addressbook) {
        try {
          ...
          console.error(error);
          showError(
            t('contacts', 'An error occurred while trying to move the contact'),
          );
        } finally {
          this.loadingUpdate = false;
        }
    ...
    async copyContactToAddressbook(addressbookId) {
      ...
      if (addressbook) {
        try {
          ...
          console.error(error);
          showError(
            t('contacts', 'An error occurred while trying to copy the contact'),
          );
        } finally {
          this.loadingUpdate = false;
        }
    ...
    async cloneContact() {
      ...
      if (
        ...
        this.addressbooks.find(
          (addressbook) => addressbook.id === this.pickedAddressbook.id,
        )
      ) {
    ...
    getLastUsedAddressBook() {
      const lastUsed = JSON.parse(
        browserStorage.getItem('last-used-addressbook') || '{}',
      );
    ...
    updateAddressBookAccesses(newAddressBook) {
      let lastUsedAddressBook = JSON.parse(
        browserStorage.getItem('last-used-addressbook') || '{}',
      );
