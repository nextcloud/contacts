...
export default class Contact {
   ...
   */
  get key() {
    return Buffer.from(this.uid + '~' + this.addressbook.id, 'utf8').toString(
      'base64',
    );
  }
  ...
  get kind() {
    return this.firstIfArray(
      ContactKindProperties.map((s) => s.toLowerCase())
   ...
   */
  get searchData() {
    const MinimalContactPropertiesLower = MinimalContactProperties.map((prop) =>
      prop.toLowerCase(),
    );
