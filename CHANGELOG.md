## [8.4.6](https://github.com/nextcloud/contacts/compare/v8.4.5...v8.4.6) (2026-05-12)


### Bug Fixes

* **deps:** Fix npm audit ([2a1b80b](https://github.com/nextcloud/contacts/commit/2a1b80bb9a301719050315395cc0d79083b5bae7))
* **l10n:** Update translations from Transifex ([5904320](https://github.com/nextcloud/contacts/commit/590432040cb20aaaa715438932f893437a7cf170))
* **l10n:** Update translations from Transifex ([fb09522](https://github.com/nextcloud/contacts/commit/fb09522e754910f5c87b7e914fa3d8aa7ae73c31))
* **l10n:** Update translations from Transifex ([f69f233](https://github.com/nextcloud/contacts/commit/f69f233481fcd14cd5fd7fba22cc191136d4cf20))
* **l10n:** Update translations from Transifex ([bc657b5](https://github.com/nextcloud/contacts/commit/bc657b56a8ad03c016c186cf864fb1190abbd431))
* **l10n:** Update translations from Transifex ([f2b54cf](https://github.com/nextcloud/contacts/commit/f2b54cf32c00e7a351e9a4da816788b928a5c774))



## [8.4.5](https://github.com/nextcloud/contacts/compare/v8.4.4...v8.4.5) (2026-04-28)


### Bug Fixes

* **deps:** Fix npm audit ([35c7aee](https://github.com/nextcloud/contacts/commit/35c7aeeee6be632bf0f263d2de2f2568fce4d899))
* **deps:** Fix npm audit ([a1ed696](https://github.com/nextcloud/contacts/commit/a1ed696ec6c0d30670be0e9b2f40f79e11c174ae))
* **deps:** update dependency uuid to v14 ([a2366f7](https://github.com/nextcloud/contacts/commit/a2366f74314fec799ec134e782126e7937ae25a8))
* Fix Teams description in nav. Shorten text, fix alignment, improve text color ([f3b521e](https://github.com/nextcloud/contacts/commit/f3b521e9318f1e8ae171eb2ead06c8c9168040dd))
* **l10n:** Update translations from Transifex ([7cefaef](https://github.com/nextcloud/contacts/commit/7cefaef6a974b414b50729045501e88cdffd8878))
* **l10n:** Update translations from Transifex ([3b539f3](https://github.com/nextcloud/contacts/commit/3b539f3e737593eef35824dbc179900c6dbe12f5))
* **l10n:** Update translations from Transifex ([b4a4684](https://github.com/nextcloud/contacts/commit/b4a4684845add97ecb84356e495855eced6035c3))
* **l10n:** Update translations from Transifex ([ccc720c](https://github.com/nextcloud/contacts/commit/ccc720c71967502d3e293d206ec018b68609d932))
* **l10n:** Update translations from Transifex ([b46b629](https://github.com/nextcloud/contacts/commit/b46b629635152122dd21874b88196e7afc5696ed))
* **l10n:** Update translations from Transifex ([6ecb516](https://github.com/nextcloud/contacts/commit/6ecb51629d48d3957f0c729355440534103f0ef2))
* **SocialApi:** Ensure the address book permissions are sufficent before doing update ([0ffa345](https://github.com/nextcloud/contacts/commit/0ffa345ad3fe7bf66131bb91cf9c76db2868ddde))



## [8.4.4](https://github.com/nextcloud/contacts/compare/v8.4.3...v8.4.4) (2026-04-14)


### Bug Fixes

* **deps:** Fix npm audit ([3e85e26](https://github.com/nextcloud/contacts/commit/3e85e2620134b2488103afba4b10519185b9c395))
* **l10n:** Update translations from Transifex ([6067ad7](https://github.com/nextcloud/contacts/commit/6067ad7eb3300ed4078d97ed6bfe2426af8bd3c2))
* **l10n:** Update translations from Transifex ([67b8a76](https://github.com/nextcloud/contacts/commit/67b8a7669dc35527b1c7ed7937032a5ca71f9e74))
* **l10n:** Update translations from Transifex ([b13a829](https://github.com/nextcloud/contacts/commit/b13a829c65843d212ecb615d86549d00e96a26e4))
* **l10n:** Update translations from Transifex ([9daa8c1](https://github.com/nextcloud/contacts/commit/9daa8c19b146004f849776aec3a6ed52a77a32b9))
* **l10n:** Update translations from Transifex ([4b1e2aa](https://github.com/nextcloud/contacts/commit/4b1e2aab685f7ac1595b3caec636de97189c276e))
* **l10n:** Update translations from Transifex ([f61fdb7](https://github.com/nextcloud/contacts/commit/f61fdb7b2eee2ca5d44137522ec1700615846396))
* **l10n:** Update translations from Transifex ([491da87](https://github.com/nextcloud/contacts/commit/491da87d64d922408ebaeb34da16edc5809f3417))



## [8.4.3](https://github.com/nextcloud/contacts/compare/v8.4.2...v8.4.3) (2026-03-31)


### Bug Fixes

* **deps:** Fix npm audit ([7bfff2e](https://github.com/nextcloud/contacts/commit/7bfff2e53d801aa39bbf0c1660d723a1071e1196))
* error message "Unable to delete team" ([1c21941](https://github.com/nextcloud/contacts/commit/1c219418ecd9b92c95d366229d2a6780b3989599)), closes [#4680](https://github.com/nextcloud/contacts/issues/4680)
* **l10n:** Update translations from Transifex ([31af062](https://github.com/nextcloud/contacts/commit/31af0620ec0de3b02f91091edee9c56a4f26a499))
* **l10n:** Update translations from Transifex ([78265b6](https://github.com/nextcloud/contacts/commit/78265b6c9dc26c786abb26f3ab5b50ad992053cd))
* **l10n:** Update translations from Transifex ([81a3c3f](https://github.com/nextcloud/contacts/commit/81a3c3f93447908f631bcbfc4b3506f43acdc1da))
* **l10n:** Update translations from Transifex ([fde23fd](https://github.com/nextcloud/contacts/commit/fde23fd9c66567e460bfa4f198512ff030bce783))
* **MemberList:** implement lazy loading ([0ed5015](https://github.com/nextcloud/contacts/commit/0ed5015931e12c45950c9eaa1ac2558926074f42))
* **PropertyDateTime:** date formatting ([8c9a3ce](https://github.com/nextcloud/contacts/commit/8c9a3cea3f589e9d087ca070e78cc5447bc8ec7c))



## [8.4.2](https://github.com/nextcloud/contacts/compare/v8.4.1...v8.4.2) (2026-03-25)


### Bug Fixes

* **deps:** Fix npm audit ([1d0d035](https://github.com/nextcloud/contacts/commit/1d0d035489a81cff1ef0a8ec8d667c15b7446293))
* **deps:** Fix npm audit ([44bed8a](https://github.com/nextcloud/contacts/commit/44bed8a56c56b893e542bc65fd99b4e6d5db1fe9))
* dont allow javascript uri as website ([c273ba5](https://github.com/nextcloud/contacts/commit/c273ba53c30acfca96fe505f107cd511a4179b56))
* **l10n:** Update translations from Transifex ([a9174d2](https://github.com/nextcloud/contacts/commit/a9174d2613a48ede2036e92e2a777cd142b14053))
* **l10n:** Update translations from Transifex ([a4d5c6a](https://github.com/nextcloud/contacts/commit/a4d5c6ab952dc27848f84dfadb163ccaa55ddb47))
* **l10n:** Update translations from Transifex ([70d04a7](https://github.com/nextcloud/contacts/commit/70d04a78bac28667534e3d7348b29f689350d8b6))
* **l10n:** Update translations from Transifex ([f600466](https://github.com/nextcloud/contacts/commit/f600466df34e70a4fc9fc060b6c54a6001d6a88a))
* **l10n:** Update translations from Transifex ([268cd0c](https://github.com/nextcloud/contacts/commit/268cd0cf99c9d1cf5d1f50f8469fca9f03679538))
* **l10n:** Update translations from Transifex ([e307ce1](https://github.com/nextcloud/contacts/commit/e307ce12cf39c684e80a34c55f786fd0974605df))
* **l10n:** Update translations from Transifex ([5ab0050](https://github.com/nextcloud/contacts/commit/5ab0050a12125a1ec8e1e1e452b054febc077c4d))
* **l10n:** Update translations from Transifex ([ac6e5a0](https://github.com/nextcloud/contacts/commit/ac6e5a0fef96ebe35656bf7ac2c96bc04d640121))
* **l10n:** Update translations from Transifex ([d1b862b](https://github.com/nextcloud/contacts/commit/d1b862bfb6bfd83a9cdb8092ef73b35c57fbf30d))
* **l10n:** Update translations from Transifex ([e70dfa5](https://github.com/nextcloud/contacts/commit/e70dfa5db4d927de399b10c4c0f0d3fa51eb13f9))
* **l10n:** Update translations from Transifex ([bfea823](https://github.com/nextcloud/contacts/commit/bfea82335044f2f45fe03e4df2b3bfabbdeedbc7))
* **l10n:** Update translations from Transifex ([f45ea4a](https://github.com/nextcloud/contacts/commit/f45ea4aa2d62d34e7b0834f2547357c24d39aa98))



## [8.4.1](https://github.com/nextcloud/contacts/compare/v8.4.0...v8.4.1) (2026-03-11)


### Bug Fixes

* **a11y:** add missing input lables ([fadd6a6](https://github.com/nextcloud/contacts/commit/fadd6a6c5e4ee3900471bef01d0d06f6f796c6e3))
* **a11y:** add missing link text ([09a2a6e](https://github.com/nextcloud/contacts/commit/09a2a6ec0dc71783acbfe74d70037e9626f227d5))
* **a11y:** switch contacts settings to a button and preserve styling ([2749128](https://github.com/nextcloud/contacts/commit/27491286f57f75820c2f17e452576213dd3a9529))
* **a11y:** turn teams description into a list item ([6489048](https://github.com/nextcloud/contacts/commit/648904840c5aadd70217537d6e312a44e1614230))
* **a11y:** wrap list items in ul ([e8cf793](https://github.com/nextcloud/contacts/commit/e8cf793693bff5be5e79e63b731d8edd6a19fead))
* add member in circles view instead of trying to add contacts ([0c86b28](https://github.com/nextcloud/contacts/commit/0c86b28fcc5748f88aec841d58e49bcd2e1297b1))
* add missing await ([291af05](https://github.com/nextcloud/contacts/commit/291af0589a0af6d9106e1a7c4f1e00e57e4d0e31))
* add missing translations ([30fda17](https://github.com/nextcloud/contacts/commit/30fda17afaf5b77fbe16a77b3358a16e55393322))
* adjust import files action to vue 3 ([6c9275f](https://github.com/nextcloud/contacts/commit/6c9275f231e6c1d134fc13bc21620c37a6d2f25b))
* adjust spacing and alignment in the contact details view ([75ee842](https://github.com/nextcloud/contacts/commit/75ee842cd90db8c6436c18855090b4319c442f75))
* Adjust styling of team resource images ([8b65d58](https://github.com/nextcloud/contacts/commit/8b65d581e5ce6ddffa75695f141abda824dd12ce))
* Allow dynamic autoloading for classes added during upgrade ([08e5f9e](https://github.com/nextcloud/contacts/commit/08e5f9e9241c1162b277764d6711823f3b0e7182))
* allow team owners to transfer ownership ([1e74d78](https://github.com/nextcloud/contacts/commit/1e74d78865c252c2136739b961823f5b161a7d34))
* always convert vcard photo urls to string ([12bdf42](https://github.com/nextcloud/contacts/commit/12bdf42772b0c7f7dfad1e0815e5179ca4a42925))
* **Batch:** incorrect translations import ([547ac22](https://github.com/nextcloud/contacts/commit/547ac228355b60b15fe4eab7f0565e20142a14f9))
* bundle moment.js locale data ([9fb1d09](https://github.com/nextcloud/contacts/commit/9fb1d09b20a6b702da000431b44ededce9a1f35a))
* center align rows ([f7a43ba](https://github.com/nextcloud/contacts/commit/f7a43bad43bbc526deacf8f40c8c3c08e688b461))
* change wording from disabled to hidden for addressBooks ([9334525](https://github.com/nextcloud/contacts/commit/93345254e774aef157e7903ce08a31ece58076c6))
* changing addressbook of contact ([1f94d47](https://github.com/nextcloud/contacts/commit/1f94d472b5c661ca364a7d268f30756fa029af11))
* check addressbook permissions before importing contacts ([5b2b17a](https://github.com/nextcloud/contacts/commit/5b2b17af165da2a042d2904c295285147fbba8da))
* check if photo property is set before downloading ([f33c353](https://github.com/nextcloud/contacts/commit/f33c353ce92096a5c715a75a1c4fd8629eed932b))
* **CircleDetails:** Improve calculation of member list ([05be5e3](https://github.com/nextcloud/contacts/commit/05be5e320697d45371c7a91ae07e31196e931830))
* **CircleDetails:** Use CheckboxRadioSwitch from @nextcloud/vue 9.x.x ([4cff865](https://github.com/nextcloud/contacts/commit/4cff865e744cefd80d4eb56bcb058a3924c31cf1))
* **CircleNavigationItem:** Rename 'add member' to 'manage team' ([eda53a5](https://github.com/nextcloud/contacts/commit/eda53a564d23bc87d9c5bfd614e5ddc8f7fe4777))
* **circles:** misaligned member modal headings ([a5ec367](https://github.com/nextcloud/contacts/commit/a5ec3675565a450b5b2da09b5b97714df059d483))
* **circles:** sort fallback if not member of circle ([05f04df](https://github.com/nextcloud/contacts/commit/05f04df69e9c09d9f4443e11d53601f9d2381d50))
* clarify addressbook sharing permissions ([0ef19f5](https://github.com/nextcloud/contacts/commit/0ef19f595b284fac90a51699df79f3bd12cfabbc))
* client integration: correct tooltip format ([8e1f68c](https://github.com/nextcloud/contacts/commit/8e1f68c1dfb0370f1c1eb48e3616cdd671892de5))
* close menu after selecting an option ([4b18d13](https://github.com/nextcloud/contacts/commit/4b18d132acdfe642c040d80fae95caf841cd6422))
* **contact:** Add normalized phone numbers to searchData ([af289b6](https://github.com/nextcloud/contacts/commit/af289b667ddac7d28112bab9ff5fe1b70345a397))
* **ContactDetailsAvatar:** border radius cropping too much ([93d850e](https://github.com/nextcloud/contacts/commit/93d850e89a0e121916f071dfc41970cd13c985b0))
* **ContactDetailsProperty:** query selector causing errors ([74d468c](https://github.com/nextcloud/contacts/commit/74d468ce8a63683d95998fbf40b24f53dd4277ba))
* **ContactDetails:** wrong box shadow on inputs ([1643248](https://github.com/nextcloud/contacts/commit/1643248b2158f695de0723115a9c99eb2907dd01))
* **contact:** don't add metadata properties and property names to searchData ([4cb4b50](https://github.com/nextcloud/contacts/commit/4cb4b50e6a76997d2b49fc752697b48e617ab753)), closes [#1176](https://github.com/nextcloud/contacts/issues/1176)
* **contact:** include phone numbers when getting contacts for address book ([00139b6](https://github.com/nextcloud/contacts/commit/00139b6c9a122a6eac4eea21386a0715d68a761b)), closes [#3112](https://github.com/nextcloud/contacts/issues/3112)
* **contacts:** add confirmation dialog when leaving a team ([0e8fd86](https://github.com/nextcloud/contacts/commit/0e8fd868ad2d740089fb6599b121bc662bfd5d49)), closes [#4458](https://github.com/nextcloud/contacts/issues/4458)
* **contactsDetails:** remove inaccessible code ([7375f02](https://github.com/nextcloud/contacts/commit/7375f023622dc3e44ed4f578095c3d303704c193))
* **ContactsList:** don't choke when filtering for `+` ([5cb0d6d](https://github.com/nextcloud/contacts/commit/5cb0d6d4c8fe56f107195906f0bee8982f6d2f3e))
* **ContactsList:** make scrolling on mobile work again ([9ef361b](https://github.com/nextcloud/contacts/commit/9ef361be9ad6d146d3ea9517c3175993b90ad2aa))
* **ContactsList:** not being visible on small screens ([5d0341e](https://github.com/nextcloud/contacts/commit/5d0341eda87150c69655db892b2c76a859505ff4))
* **contacts:** split up detailed-name again to fix vCard ([501b990](https://github.com/nextcloud/contacts/commit/501b9905abdf55f45f3d67b72680c2cf6918b13c))
* convert pre-commit to common js ([afd5a5a](https://github.com/nextcloud/contacts/commit/afd5a5a0600e0bfb9836e137105e3ede8cd7d749))
* convert pre-commit.js to ES module ([6f06719](https://github.com/nextcloud/contacts/commit/6f06719e8b7740ca16b901b8fc0d9af8d993b462))
* **copy:** Hide not copyable addressbooks in copy select ([b588aac](https://github.com/nextcloud/contacts/commit/b588aac99118e649c02abb3ac0cd1dc5e7ac3468))
* **declarative-ui:** rename to client_integration ([70f9712](https://github.com/nextcloud/contacts/commit/70f97120bea241e80c92873c2dea1d71884537ee))
* Decode HTML entities in team display names ([7fc1649](https://github.com/nextcloud/contacts/commit/7fc1649a57475608ce7b1b7fd25f5f11b2118f0e))
* **deps:** add missing picomatch@4.0.3 to lock file ([1d598e2](https://github.com/nextcloud/contacts/commit/1d598e29ba320527c7d9e12f213a61db4db11117))
* **deps:** add missing picomatch@4.0.3 to lock file again ([216aa4d](https://github.com/nextcloud/contacts/commit/216aa4dd41af367311c68ad3feb541c7a337de1b))
* **deps:** bump @mattkrick/sanitize-svg from 0.4.0 to ^0.4.1 ([deaa81b](https://github.com/nextcloud/contacts/commit/deaa81bf7bf4af95b129f8d2513c9456642cbaa4))
* **deps:** bump @nextcloud/auth from 2.3.0 to ^2.4.0 ([ef072de](https://github.com/nextcloud/contacts/commit/ef072deebc3b057a9dd43466ec88c90df96a5b89))
* **deps:** bump @nextcloud/auth from 2.4.0 to ^2.5.1 ([a8a5745](https://github.com/nextcloud/contacts/commit/a8a5745466fe08af7abe7412fb1eaf97ca5aa746))
* **deps:** bump @nextcloud/auth from 2.5.1 to ^2.5.2 ([7f02252](https://github.com/nextcloud/contacts/commit/7f022525ae2acdbd83f732f9842dcb8172e8714c))
* **deps:** bump @nextcloud/auth from 2.5.2 to ^2.5.3 ([7406cb7](https://github.com/nextcloud/contacts/commit/7406cb709edbd702b16e264417bfc4813cb889ae))
* **deps:** bump @nextcloud/axios from 2.5.0 to ^2.5.1 ([85f127a](https://github.com/nextcloud/contacts/commit/85f127af7627e2e6714f6ed6632bb84f79bee73a))
* **deps:** bump @nextcloud/axios from 2.5.1 to ^2.5.2 ([cb77d56](https://github.com/nextcloud/contacts/commit/cb77d569f73de7597d582efcc8ead10dd2a5e68c))
* **deps:** bump @nextcloud/browser-storage from 0.4.0 to ^0.5.0 ([c894ba5](https://github.com/nextcloud/contacts/commit/c894ba570ec984111888d7aa211aa5e1016838f5))
* **deps:** bump @nextcloud/cdav-library from 1.3.0 to ^1.4.0 ([a02a4b5](https://github.com/nextcloud/contacts/commit/a02a4b5d5f60e62abcf5b39cf12474a6a5d47afa))
* **deps:** bump @nextcloud/cdav-library from 1.4.0 to ^1.5.1 ([ff451d0](https://github.com/nextcloud/contacts/commit/ff451d02c0749cbe85688f6a17b49e2568efe399))
* **deps:** bump @nextcloud/cdav-library from 1.5.1 to ^1.5.2 ([0277f19](https://github.com/nextcloud/contacts/commit/0277f19a63f2080ee9270892b27e1c032c11c88e))
* **deps:** bump @nextcloud/cdav-library from 1.5.2 to ^1.5.3 ([84042a8](https://github.com/nextcloud/contacts/commit/84042a86cc84a98abc92e7d2c10fb164715a819b))
* **deps:** bump @nextcloud/cdav-library from 1.5.3 to v2 ([d465acf](https://github.com/nextcloud/contacts/commit/d465acfbca74696d1fc95044bfdb2ff72f09a223))
* **deps:** bump @nextcloud/cdav-library from 2.0.0 to ^2.1.0 ([a769432](https://github.com/nextcloud/contacts/commit/a769432056150b5be320d91a036354f8c9d39099))
* **deps:** bump @nextcloud/cdav-library from 2.1.0 to ^2.1.1 ([62f6585](https://github.com/nextcloud/contacts/commit/62f658597f4e84815502f61a35c309d1379cdda1))
* **deps:** bump @nextcloud/dialogs from 5.3.1 to ^5.3.2 ([c89c5e1](https://github.com/nextcloud/contacts/commit/c89c5e1e18a73699c16779b019359fbaf3742927))
* **deps:** bump @nextcloud/dialogs from 5.3.2 to ^5.3.3 ([f2941f8](https://github.com/nextcloud/contacts/commit/f2941f876fcc67c0b1d09a287462a430194e0f7c))
* **deps:** bump @nextcloud/dialogs from 5.3.3 to ^5.3.4 ([63bdf07](https://github.com/nextcloud/contacts/commit/63bdf07c90e807cf95bd7f1c2a2e26c7a68e02b8))
* **deps:** bump @nextcloud/dialogs from 5.3.4 to ^5.3.5 ([fd6bb23](https://github.com/nextcloud/contacts/commit/fd6bb234d140157a72e495df884d6cc9cd5e99a8))
* **deps:** bump @nextcloud/dialogs from 5.3.5 to ^5.3.7 ([2e6b7cf](https://github.com/nextcloud/contacts/commit/2e6b7cf8aee60612a66cefaf8c3415c6068f75d3))
* **deps:** bump @nextcloud/dialogs from 5.3.7 to ^5.3.8 ([a7abab3](https://github.com/nextcloud/contacts/commit/a7abab3de1b26f4f89ee3140d16dc41657297fd4))
* **deps:** bump @nextcloud/dialogs from 7.0.0 to ^7.0.1 ([5f078c9](https://github.com/nextcloud/contacts/commit/5f078c991702b59377780827f9e4a9f06b476d1b))
* **deps:** bump @nextcloud/dialogs from 7.0.0-rc.1 to ^7.0.0-rc.2 ([db9c47b](https://github.com/nextcloud/contacts/commit/db9c47bb979016e83369a9162147287f270f23fd))
* **deps:** bump @nextcloud/dialogs from 7.0.0-rc.2 to ^7.0.0-rc.3 ([ba7c516](https://github.com/nextcloud/contacts/commit/ba7c516ab25e19eea6ac0eb0ee38160f8f5757e8))
* **deps:** bump @nextcloud/dialogs from 7.0.0-rc.3 to ^7.0.0 ([caf1b41](https://github.com/nextcloud/contacts/commit/caf1b4100d35b51d94aa987b5e9a3cf7141a7141))
* **deps:** bump @nextcloud/event-bus from 3.3.1 to ^3.3.1 ([b1af001](https://github.com/nextcloud/contacts/commit/b1af00165ed4ec0292ef87a29a18c8aff8b5f1e8))
* **deps:** bump @nextcloud/event-bus from 3.3.1 to ^3.3.2 ([6a0b638](https://github.com/nextcloud/contacts/commit/6a0b638f0ad1482c1d85a05b7801a89ff84291cd))
* **deps:** bump @nextcloud/files from 3.10.0 to ^3.10.1 ([01bfdae](https://github.com/nextcloud/contacts/commit/01bfdae1a5df9e3c89b76bd3228f14278cba107a))
* **deps:** bump @nextcloud/files from 3.10.1 to ^3.10.2 ([1a3a6a5](https://github.com/nextcloud/contacts/commit/1a3a6a59c4d5a35c723248b8adad763921bdeb2d))
* **deps:** bump @nextcloud/files from 3.10.2 to ^3.11.0 ([70253f6](https://github.com/nextcloud/contacts/commit/70253f6f84832c366726d093cab093f6fbdad2f6))
* **deps:** bump @nextcloud/files from 3.11.0 to ^3.12.0 ([3ded977](https://github.com/nextcloud/contacts/commit/3ded9770d858896afdd27feabfa360575102e68d))
* **deps:** bump @nextcloud/files from 3.4.1 to ^3.4.1 ([cb81009](https://github.com/nextcloud/contacts/commit/cb810095c4d3ba91e4ca1a1f568a14b3210f2e56))
* **deps:** bump @nextcloud/files from 3.4.1 to ^3.5.0 ([718a090](https://github.com/nextcloud/contacts/commit/718a0900392d77f081fad22838d39e7d6950a710))
* **deps:** bump @nextcloud/files from 3.5.0 to ^3.5.1 ([ef312d3](https://github.com/nextcloud/contacts/commit/ef312d362319eb78152886487f0428d0301b7687))
* **deps:** bump @nextcloud/files from 3.5.1 to ^3.6.0 ([770b27c](https://github.com/nextcloud/contacts/commit/770b27c738763358faf3c6ecb6bcb8568ad01895))
* **deps:** bump @nextcloud/files from 3.8.0 to ^3.8.0 ([769a947](https://github.com/nextcloud/contacts/commit/769a94783446ea64d9e94b23e4896eca2e3718ef))
* **deps:** bump @nextcloud/files from 3.8.0 to ^3.9.0 ([3a016b2](https://github.com/nextcloud/contacts/commit/3a016b2655b04df0f916ecc5b4a18182e18e52d8))
* **deps:** bump @nextcloud/files from 3.9.1 to ^3.10.0 ([498fb12](https://github.com/nextcloud/contacts/commit/498fb127eb9690d96c0c8bb51f8c243a09a7a166))
* **deps:** bump @nextcloud/files from 3.9.1 to ^3.9.1 ([1e086f9](https://github.com/nextcloud/contacts/commit/1e086f91bb0763e9e618f302108d7334ab2433bb))
* **deps:** bump @nextcloud/initial-state from 2.2.0 to v3 ([b2ea05d](https://github.com/nextcloud/contacts/commit/b2ea05db7cf49168fbf94a77de7fa0ca0568b64f))
* **deps:** bump @nextcloud/l10n from 2.2.0 to v3 ([df25646](https://github.com/nextcloud/contacts/commit/df25646a5392d66b648bd83b3265160bf417ea6e))
* **deps:** bump @nextcloud/l10n from 3.1.0 to v3.4.0 ([529c5f1](https://github.com/nextcloud/contacts/commit/529c5f1f6ab35aeb0187167af3582e3b4ae15911))
* **deps:** bump @nextcloud/logger from 2.7.0 to v3 ([73eecbc](https://github.com/nextcloud/contacts/commit/73eecbcde459436095a23bb7b2c9773b8d18bc96))
* **deps:** bump @nextcloud/moment from 1.3.1 to ^1.3.2 ([41c601c](https://github.com/nextcloud/contacts/commit/41c601cf58ac7b61db7ac42b1b64380e37dc4531))
* **deps:** bump @nextcloud/moment from 1.3.2 to ^1.3.4 ([3f2029b](https://github.com/nextcloud/contacts/commit/3f2029b6abb6e1d3b05b3c479cb8a28258ec78f8))
* **deps:** bump @nextcloud/moment from 1.3.4 to ^1.3.5 ([abf1a49](https://github.com/nextcloud/contacts/commit/abf1a49ddf3debe5d91b64bc050239beecbb4f48))
* **deps:** bump @nextcloud/paths from 2.2.1 to ^2.2.1 ([9b6c6d3](https://github.com/nextcloud/contacts/commit/9b6c6d38ba88f2dd69b65a5a0587db98adc02ca5))
* **deps:** bump @nextcloud/router from 2.2.0 to v3 ([96af3c5](https://github.com/nextcloud/contacts/commit/96af3c573aa4300ca258961046e8b149f58de3c6))
* **deps:** bump @nextcloud/sharing from 0.1.0 to ^0.2.2 ([29c3f76](https://github.com/nextcloud/contacts/commit/29c3f7603722548160a994262f5df616534686ca))
* **deps:** bump @nextcloud/sharing from 0.2.3 to ^0.2.3 ([03d3efc](https://github.com/nextcloud/contacts/commit/03d3efc0a1b85117cf0609c8a1d9cad7b8c025d4))
* **deps:** bump @nextcloud/sharing from 0.2.4 to ^0.2.4 ([c47b8e7](https://github.com/nextcloud/contacts/commit/c47b8e7ffc71e7641d55a8a65835f6a6f7c8a1fa))
* **deps:** bump @nextcloud/sharing from 0.2.4 to ^0.2.5 ([1c748b4](https://github.com/nextcloud/contacts/commit/1c748b419fcb611007399a02287f9310328ad8a9))
* **deps:** bump @nextcloud/sharing from 0.2.5 to ^0.3.0 ([1e36a5e](https://github.com/nextcloud/contacts/commit/1e36a5efcc52eb3a44ae6e50ce48ff297ccafb87))
* **deps:** bump @nextcloud/vue from 8.11.3 to ^8.12.0 ([21dbcf5](https://github.com/nextcloud/contacts/commit/21dbcf5653b9fdd11da99f5cc88ddb8a7636ba88))
* **deps:** bump @nextcloud/vue from 8.12.0 to ^8.13.0 ([56fc63c](https://github.com/nextcloud/contacts/commit/56fc63c07b1f2c69c25433c1f896f9fecd18f6d4))
* **deps:** bump @nextcloud/vue from 8.13.0 to ^8.14.0 ([5d85e20](https://github.com/nextcloud/contacts/commit/5d85e2098ca802b7998535618e4c76009197f0f9))
* **deps:** bump @nextcloud/vue from 8.14.0 to ^8.15.0 ([5dc3c14](https://github.com/nextcloud/contacts/commit/5dc3c1419c38b05f9eac5f7c8f9d63a540a40724))
* **deps:** bump @nextcloud/vue from 8.15.0 to ^8.15.1 ([0919189](https://github.com/nextcloud/contacts/commit/0919189714270b519fe2fd7cedd695ca230bf99a))
* **deps:** bump @nextcloud/vue from 8.15.1 to ^8.18.0 ([096dd07](https://github.com/nextcloud/contacts/commit/096dd0713df368cc86ce7dc36aa55e4740ec6750))
* **deps:** bump @nextcloud/vue from 8.18.0 to ^8.19.0 ([f59b0f2](https://github.com/nextcloud/contacts/commit/f59b0f2dd13292262a474d12e9e0eb2bdd85891c))
* **deps:** bump @nextcloud/vue from 8.19.0 to ^8.22.0 ([a7d4203](https://github.com/nextcloud/contacts/commit/a7d4203a843e846d1d8f2b3a21823d432f71f33c))
* **deps:** bump @nextcloud/vue from 8.22.0 to ^8.23.1 ([3d343ad](https://github.com/nextcloud/contacts/commit/3d343ada3c097536cedde0f9978e8cf8553c1cea))
* **deps:** bump @nextcloud/vue from 8.24.0 to ^8.25.0 ([0e5cf05](https://github.com/nextcloud/contacts/commit/0e5cf057475475e50dd846e26c89d5689e6cc1e5))
* **deps:** bump @nextcloud/vue from 8.25.0 to ^8.25.1 ([3e35686](https://github.com/nextcloud/contacts/commit/3e356860bdc9e1f508c644729930782a085a1aa7))
* **deps:** bump @nextcloud/vue from 8.25.1 to ^8.26.0 ([e071c4a](https://github.com/nextcloud/contacts/commit/e071c4ac38f39f9146f5045ad06cd5c1d969553e))
* **deps:** bump @nextcloud/vue from 8.26.1 to ^8.27.0 ([2b1c724](https://github.com/nextcloud/contacts/commit/2b1c724f21916e55b1a56551d0a23cb3ab92c16d))
* **deps:** bump @nextcloud/vue from 9.0.0 to ^9.0.1 ([e073817](https://github.com/nextcloud/contacts/commit/e0738172d52a3057509aeb3782253cff3eb7acc0))
* **deps:** bump @nextcloud/vue from 9.0.0-rc.6 to ^9.0.0-rc.8 ([1d14d9e](https://github.com/nextcloud/contacts/commit/1d14d9e2fb18f86e3ed9340b78e4ede735af6310))
* **deps:** bump @nextcloud/vue from 9.0.0-rc.8 to ^9.0.0 ([8e97dfc](https://github.com/nextcloud/contacts/commit/8e97dfcf1b0e82d7a85365dc79c3a60c984e0487))
* **deps:** bump @vueuse/core from 10.11.1 to ^10.11.1 ([17b1ac7](https://github.com/nextcloud/contacts/commit/17b1ac7ab4dd0248d52cd80c44dca61fff48d6f2))
* **deps:** bump @vueuse/core from 10.11.1 to v11 ([5545596](https://github.com/nextcloud/contacts/commit/5545596592dd0b51ab3bb9e026dc8bdc97732a92))
* **deps:** bump @vueuse/core from 10.9.0 to ^10.11.0 ([09b06c9](https://github.com/nextcloud/contacts/commit/09b06c9a933974e1b8e6b6d02e1a929a9ea3f249))
* **deps:** bump @vueuse/core from 11.0.3 to ^11.1.0 ([e66804d](https://github.com/nextcloud/contacts/commit/e66804d331598e14b8499f026f3d306384f12f84))
* **deps:** bump @vueuse/core from 11.1.0 to ^11.2.0 ([8d5dad1](https://github.com/nextcloud/contacts/commit/8d5dad1683fd3ae083673e5765fa630806dc5077))
* **deps:** bump @vueuse/core from 11.2.0 to ^11.3.0 ([38470f6](https://github.com/nextcloud/contacts/commit/38470f6797641ac783ad171bf72b67eb4ed734a5))
* **deps:** bump debounce from 2.0.0 to ^2.1.0 ([1834ff5](https://github.com/nextcloud/contacts/commit/1834ff551a615545bf8d9f6adf1514093201110e))
* **deps:** bump debounce from 2.1.0 to ^2.1.1 ([81ddf6e](https://github.com/nextcloud/contacts/commit/81ddf6e59612642edbcbb73bb6765fad8ca4fb31))
* **deps:** bump debounce from 2.1.1 to ^2.2.0 ([5d2d79c](https://github.com/nextcloud/contacts/commit/5d2d79c963ac5d7d86d799ebdd6c2dc0e988d80b))
* **deps:** bump form-data to 4.0.4 ([10c4691](https://github.com/nextcloud/contacts/commit/10c46912cd928b41605e298116adbbf4071678d0))
* **deps:** bump ical.js from 1.5.0 to v2 ([ef70705](https://github.com/nextcloud/contacts/commit/ef70705cb84b033e33dab46f68f94c947deed73b))
* **deps:** bump ical.js from 2.0.1 to ^2.1.0 ([5ae2289](https://github.com/nextcloud/contacts/commit/5ae2289795d5cddacc89373f7c330cb642ed742a))
* **deps:** bump ical.js from 2.1.0 to ^2.2.0 ([597506e](https://github.com/nextcloud/contacts/commit/597506e069483cfed949a3f578a350fc1d8efb48))
* **deps:** bump ical.js from 2.2.0 to ^2.2.1 ([3a1f686](https://github.com/nextcloud/contacts/commit/3a1f686db9d7f278f049deaff5d661864db64351))
* **deps:** bump p-limit from 5.0.0 to v6 ([f8996aa](https://github.com/nextcloud/contacts/commit/f8996aa6de495ac632542bcf71b77ac185811151))
* **deps:** bump p-limit from 6.1.0 to ^6.2.0 ([37a29a4](https://github.com/nextcloud/contacts/commit/37a29a4d82f89c1c29d6f1ce69275f025594b6dd))
* **deps:** bump p-limit from 6.2.0 to v7 ([2c34ad0](https://github.com/nextcloud/contacts/commit/2c34ad0f5affbafb6207521fbf59f349fcffe9bd))
* **deps:** bump p-limit from 7.0.0 to ^7.1.0 ([e81f587](https://github.com/nextcloud/contacts/commit/e81f587013dc70298596a41f9e14fd25423faa61))
* **deps:** bump p-limit from 7.1.0 to ^7.1.1 ([6a016ce](https://github.com/nextcloud/contacts/commit/6a016ce0571982d1df38911d9edde15921e91722))
* **deps:** bump pinia from 2.1.7 to ^2.2.2 ([5e0ec67](https://github.com/nextcloud/contacts/commit/5e0ec67c3321c5e9f412f8c79e4aaaadc40953f0))
* **deps:** bump pinia from 2.2.2 to ^2.2.4 ([92bcb60](https://github.com/nextcloud/contacts/commit/92bcb60ade7c742a25aad66f3906787ee3e74cde))
* **deps:** bump pinia from 2.2.4 to ^2.2.6 ([81d78eb](https://github.com/nextcloud/contacts/commit/81d78eb0fc9127e3b745082f2ab4341a92dc4eb7))
* **deps:** bump pinia from 2.2.6 to ^2.2.8 ([1fe78fe](https://github.com/nextcloud/contacts/commit/1fe78fe1d9fb0616c9b94c7c4e3a37c55b0e8a1f))
* **deps:** bump pinia from 2.2.8 to ^2.3.0 ([04a1d6f](https://github.com/nextcloud/contacts/commit/04a1d6fc2019d7d5c6f09cfc6ae1e7e8d1dbe3de))
* **deps:** bump pinia from 2.3.0 to ^2.3.1 ([1940948](https://github.com/nextcloud/contacts/commit/1940948208165075defd04a44d9462b167279262))
* **deps:** bump qreator from 9.7.1 to ^9.7.2 ([eee9866](https://github.com/nextcloud/contacts/commit/eee9866979cf3cfbe4983aa89e350fa5e283bcb3))
* **deps:** bump uuid from 10.0.0 to v11 ([35dbc90](https://github.com/nextcloud/contacts/commit/35dbc9005f7c35228c9bc19f432bda4bc7e6708d))
* **deps:** bump uuid from 11.0.2 to ^11.0.3 ([a854c30](https://github.com/nextcloud/contacts/commit/a854c305e3f1f75477c7a771297546e1476c3b1a))
* **deps:** bump uuid from 11.0.3 to ^11.0.5 ([546148d](https://github.com/nextcloud/contacts/commit/546148d1974c521c78c0b1fd3a68c7068a37c077))
* **deps:** bump uuid from 11.0.5 to ^11.1.0 ([29f006c](https://github.com/nextcloud/contacts/commit/29f006c9afdfbf03d1eca7566d5508a8549cc68a))
* **deps:** bump uuid from 11.1.0 to v13 ([c859a96](https://github.com/nextcloud/contacts/commit/c859a96dea8a1264e7716d761b95681eb1fdf720))
* **deps:** bump uuid from 9.0.1 to v10 ([b350b27](https://github.com/nextcloud/contacts/commit/b350b2781f3b4e394e0ea80926c127b401defb8d))
* **deps:** bump validator from 13.12.0 to ^13.15.0 ([5f48b34](https://github.com/nextcloud/contacts/commit/5f48b3403ba6a45a5c946ee9961a559ca087e6be))
* **deps:** bump validator from 13.15.0 to ^13.15.15 ([cf3db85](https://github.com/nextcloud/contacts/commit/cf3db8577db81c44c9272794228b2af76c43ec9c))
* **deps:** bump virtua from 0.41.5 to ^0.42.3 ([5025d98](https://github.com/nextcloud/contacts/commit/5025d98e9179f0ff7db17cfc849cafe33652c7b0))
* **deps:** bump virtua from 0.42.3 to ^0.44.2 ([db9b88d](https://github.com/nextcloud/contacts/commit/db9b88d8cb1a9022095018c8462803954758e331))
* **deps:** bump virtua from 0.44.2 to ^0.44.3 ([aec0b43](https://github.com/nextcloud/contacts/commit/aec0b431de2a136b8b0cc2b1de909c96ffa46910))
* **deps:** bump virtua from 0.44.3 to ^0.45.3 ([2190da1](https://github.com/nextcloud/contacts/commit/2190da152c2b40017dee6c825b6c4230e2d6b45f))
* **deps:** bump vite from 5.4.14 to v5.4.15 ([8b5092f](https://github.com/nextcloud/contacts/commit/8b5092fcd570c2488dcbda07535d7c7b57bf9358))
* **deps:** bump vite from 5.4.17 to v5.4.18 ([6b7ffdf](https://github.com/nextcloud/contacts/commit/6b7ffdf1fdd12afc2cb14b13daa703d2dbacd069))
* **deps:** bump vite from 5.4.19 to v5.4.20 ([b75e07e](https://github.com/nextcloud/contacts/commit/b75e07eef87f512852350b445c4086fa3da8156d))
* **deps:** bump vue monorepo from 2.7.16 to ~2.7.16 ([bda5244](https://github.com/nextcloud/contacts/commit/bda5244373d44f82e6eb2f5ed1fc302bf5968b0e))
* **deps:** bump vue-material-design-icons from 5.3.0 to ^5.3.1 ([b94efd8](https://github.com/nextcloud/contacts/commit/b94efd8c389c3508de9113269e9cf1687517d243))
* **deps:** fix npm audit ([7d53f6c](https://github.com/nextcloud/contacts/commit/7d53f6c45b90be54dcb09016682531c7c3794991))
* **deps:** fix npm audit ([85197a0](https://github.com/nextcloud/contacts/commit/85197a0eb40ab9d16233ce2989dc3862047e17da))
* **deps:** Fix npm audit ([9ef5dcc](https://github.com/nextcloud/contacts/commit/9ef5dcc452a89a48ae49eace69335d63ef6685cb))
* **deps:** Fix npm audit ([8d2b7cf](https://github.com/nextcloud/contacts/commit/8d2b7cf99f949115075c044e69dee2a5f815b75c))
* **deps:** Fix npm audit ([da2366f](https://github.com/nextcloud/contacts/commit/da2366f3e50c4197f70ba95ce9d7b7a408c55ce3))
* **deps:** Fix npm audit ([a6a1367](https://github.com/nextcloud/contacts/commit/a6a136717520d91137f6f76efdf979b7aaa60a5a))
* **deps:** Fix npm audit ([637a3c1](https://github.com/nextcloud/contacts/commit/637a3c1c59d4b8143bebc7c7416769b6aa6a8824))
* **deps:** Fix npm audit ([e4ddb5c](https://github.com/nextcloud/contacts/commit/e4ddb5cdab23e1d7df2825f619beaac36ede8267))
* **deps:** Fix npm audit ([9da1e4e](https://github.com/nextcloud/contacts/commit/9da1e4e64902a3129c645f29cb83bd2058ff3b44))
* **deps:** Fix npm audit ([3ae1e7a](https://github.com/nextcloud/contacts/commit/3ae1e7ab6ce465c5ae2c0281de1648efe44460f1))
* **deps:** Fix npm audit ([4eaf4ce](https://github.com/nextcloud/contacts/commit/4eaf4ce3d0333203976e26a1c546653be38e647b))
* **deps:** Fix npm audit ([da63d3f](https://github.com/nextcloud/contacts/commit/da63d3f61e87a9cbc902d9fa700c6825fa3e8b3b))
* **deps:** Fix npm audit ([a70a5d7](https://github.com/nextcloud/contacts/commit/a70a5d76cff7d8d83c54d83b416afe3878ab7d0a))
* **deps:** Fix npm audit ([f3087bc](https://github.com/nextcloud/contacts/commit/f3087bc5e5df5bccaa614fa51ce7bc20d63ddf1d))
* **deps:** Fix npm audit ([c46b418](https://github.com/nextcloud/contacts/commit/c46b418658c1bc1d35403dd3f4641e8e4c45e493))
* **deps:** update dependency @nextcloud/capabilities to ^1.2.1 ([040c14b](https://github.com/nextcloud/contacts/commit/040c14b63a8fa954dfb9737cd5ed730d9e363cd8))
* **deps:** update dependency @nextcloud/dialogs to ^7.1.0 ([c61b824](https://github.com/nextcloud/contacts/commit/c61b8242543238e7a1b31495b3d2d3b5042f98e8))
* **deps:** update dependency @nextcloud/dialogs to ^7.2.0 ([3811c05](https://github.com/nextcloud/contacts/commit/3811c05f8d84a54a46b6a99f8e840e1367fa08d2))
* **deps:** update dependency @nextcloud/dialogs to ^7.3.0 ([c05869e](https://github.com/nextcloud/contacts/commit/c05869ef3379948da393fed6966d2c92c31e12d9))
* **deps:** update dependency @nextcloud/event-bus to ^3.3.3 ([57eda56](https://github.com/nextcloud/contacts/commit/57eda562373019dc4af21cbf3b69518600deabf4))
* **deps:** update dependency @nextcloud/files to ^3.12.1 ([4ea7749](https://github.com/nextcloud/contacts/commit/4ea7749b5aa67500c3290076bc5ed4defb7bd3da))
* **deps:** update dependency @nextcloud/files to ^3.12.2 ([19f0f58](https://github.com/nextcloud/contacts/commit/19f0f58a39078625e93ddcb2f19c247264eb7ba4))
* **deps:** update dependency @nextcloud/l10n to v3.4.1 ([b02273a](https://github.com/nextcloud/contacts/commit/b02273a928027dd1b08b46f798cf36187fe60e09))
* **deps:** update dependency @nextcloud/logger to ^3.0.3 ([e93d41d](https://github.com/nextcloud/contacts/commit/e93d41d541cf6eac85cf801d062a1627d3930554))
* **deps:** update dependency @nextcloud/paths to ^2.3.0 ([c87970c](https://github.com/nextcloud/contacts/commit/c87970c1695f3d88cde646aa6d65a6461cab50bd))
* **deps:** update dependency @nextcloud/paths to v3 ([b8298c2](https://github.com/nextcloud/contacts/commit/b8298c27f3957df91b1b486d65b68ca557576468))
* **deps:** update dependency @nextcloud/router to ^3.1.0 ([a623f57](https://github.com/nextcloud/contacts/commit/a623f571f8039761443761432c3f1416396c644c))
* **deps:** update dependency @nextcloud/vue to ^9.1.0 ([3c1344d](https://github.com/nextcloud/contacts/commit/3c1344d3b36bbdd09855473625467c277de30520))
* **deps:** update dependency @nextcloud/vue to ^9.3.0 ([9f89f0e](https://github.com/nextcloud/contacts/commit/9f89f0eca4dab9c916480b7692e56315969d02ad))
* **deps:** update dependency @nextcloud/vue to ^9.3.1 ([fa67fab](https://github.com/nextcloud/contacts/commit/fa67fab111f4da989168db560a672f78d47e59f7))
* **deps:** update dependency @nextcloud/vue to ^9.3.3 ([7f76d92](https://github.com/nextcloud/contacts/commit/7f76d92c4e971f9b99765c6dd05f3b1077155216))
* **deps:** update dependency @nextcloud/vue to ^9.5.0 ([161b7d1](https://github.com/nextcloud/contacts/commit/161b7d168cdb1019009981e03cd6ec24ae7ae5e3))
* **deps:** update dependency @vueuse/core to ^14.2.1 ([ba41211](https://github.com/nextcloud/contacts/commit/ba4121194b01c54c3744d0077f5c14d15d2c6228))
* **deps:** update dependency @vueuse/core to v14 ([15ec7d6](https://github.com/nextcloud/contacts/commit/15ec7d6296ae94f6375e1527ce495c76ad33acfc))
* **deps:** update dependency camelcase to v9 ([5c2f0c5](https://github.com/nextcloud/contacts/commit/5c2f0c5f1036d5100652f711232d050d2e9c6907))
* **deps:** update dependency debounce to v3 ([becd1f0](https://github.com/nextcloud/contacts/commit/becd1f00216ced98f9c2dd757e594611edfde509))
* **deps:** update dependency lodash to ^4.17.23 ([f2ebdce](https://github.com/nextcloud/contacts/commit/f2ebdce20b1e5e97fa7e813d88e64b8404bd8deb))
* **deps:** update dependency lodash to v4.17.23 ([81e827d](https://github.com/nextcloud/contacts/commit/81e827d25ac6f23041b2417aac22a19226419132))
* **deps:** update dependency p-limit to ^7.2.0 ([3de9552](https://github.com/nextcloud/contacts/commit/3de9552ef3b6e7f3b9769fd99432a1d364af5676))
* **deps:** update dependency p-limit to ^7.3.0 ([fc05234](https://github.com/nextcloud/contacts/commit/fc0523455950a9cc61105aac030f7ac387b06a64))
* **deps:** update dependency phpunit/phpunit to v9.6.33 ([c1a986c](https://github.com/nextcloud/contacts/commit/c1a986c00145786c6d607420b5a1ba1260a0e58e))
* **deps:** update dependency pinia to ^3.0.4 ([eb9f010](https://github.com/nextcloud/contacts/commit/eb9f010b2591e86f9f3387b7af4043913963217d))
* **deps:** update dependency validator to ^13.15.20 ([a5cc099](https://github.com/nextcloud/contacts/commit/a5cc099aab534e5f222c4465f4a6d41289a9a8a2))
* **deps:** update dependency validator to ^13.15.23 ([0df4bb7](https://github.com/nextcloud/contacts/commit/0df4bb732776d44d4c7faaf88bd230f37e075a70))
* **deps:** update dependency validator to ^13.15.26 ([ad84b1f](https://github.com/nextcloud/contacts/commit/ad84b1f8e3d1c105a31826c81c37eed6f93205fc))
* **deps:** update dependency validator to v13.15.20 ([b4510a3](https://github.com/nextcloud/contacts/commit/b4510a3a4c01c882cfbbb52a7051b178e1242181))
* **deps:** update dependency virtua to ^0.46.2 ([7b1e5e7](https://github.com/nextcloud/contacts/commit/7b1e5e7870123a8f1805f0933a3f01be722a97ce))
* **deps:** update dependency virtua to ^0.46.3 ([050dc11](https://github.com/nextcloud/contacts/commit/050dc1100d87e27c3a32d052d1839853147ad2ae))
* **deps:** update dependency virtua to ^0.46.6 ([be2096b](https://github.com/nextcloud/contacts/commit/be2096bcb3d2acec31383453c02b9c6875885156))
* **deps:** update dependency virtua to ^0.48.2 ([621fcf0](https://github.com/nextcloud/contacts/commit/621fcf0f5323ae326ff18ff69cf86b4d7bceb37e))
* **deps:** update dependency virtua to ^0.48.3 ([9b8d07d](https://github.com/nextcloud/contacts/commit/9b8d07da0da5d04caa45fe49dcc695d511344c80))
* **deps:** update dependency virtua to ^0.48.5 ([42f4813](https://github.com/nextcloud/contacts/commit/42f481373434bac6e7af0900fdb81800504fba4a))
* **deps:** update dependency virtua to ^0.48.6 ([16240f6](https://github.com/nextcloud/contacts/commit/16240f65780e33ff76f11fe3226100ef6939e3cb))
* **deps:** update dependency vite to v7.1.11 ([8b3d4de](https://github.com/nextcloud/contacts/commit/8b3d4de06042aebc2be8162cc8479d1cdf93fcf8))
* **DetailsHeader:** make accessible ([d06006e](https://github.com/nextcloud/contacts/commit/d06006ea7a313e0a8bb0ed73573fcd927c3642a3))
* detect addressbook ACL's properly ([a4a0a2f](https://github.com/nextcloud/contacts/commit/a4a0a2f30bf473831dfad69423ce8ce919aff5de))
* do not allow to exclude system accounts from the bday calendar ([77432d4](https://github.com/nextcloud/contacts/commit/77432d48e463a1dc5af19226ee6f51ae914c8361))
* do not load disabled address books ([1c0ec32](https://github.com/nextcloud/contacts/commit/1c0ec320d375643c0bdedbe673fab150e0e3eab7))
* don't immediately update a vCard when changing the avatar ([60f2f84](https://github.com/nextcloud/contacts/commit/60f2f8497fe3a1302750332412989401cde58710))
* drop google+ ([30e1db2](https://github.com/nextcloud/contacts/commit/30e1db2edf453eaa22a2a91900031c7c4caaf3c8))
* duplication of address books requests ([60cf2bc](https://github.com/nextcloud/contacts/commit/60cf2bc236039693789f7ce70795b11307f55423))
* fetch addressbook contacts when enabled ([db911d5](https://github.com/nextcloud/contacts/commit/db911d5892573bc9f9ddaee3789746565139d19b))
* fix team members colum layout in chromium based browsers ([f3c2cea](https://github.com/nextcloud/contacts/commit/f3c2cea9265fcad5101e72be62530adb880c1836))
* generate rev depending on card version ([422ad78](https://github.com/nextcloud/contacts/commit/422ad788f64651366037d8c0a712a5d2f54e47c5))
* group multiple emails or phone numbers ([7f7aebc](https://github.com/nextcloud/contacts/commit/7f7aebc33b2df7bda9ead1897137a5a5f4fdfb93))
* **GroupNavigationItem:** encode exported contact groups as UTF-8 ([7e54582](https://github.com/nextcloud/contacts/commit/7e5458245f7206e1ae91c876fe9cfbeaa8a88990))
* **groups:** show hint about adding read-only contacts to contact groups ([b5fb959](https://github.com/nextcloud/contacts/commit/b5fb959d62fbbb03e9d53a9b6102eef647f7b440))
* **groups:** show read-only contacts hint only for contact groups ([e71d652](https://github.com/nextcloud/contacts/commit/e71d652a6c6250e169875779e20a4dc84f32b168))
* **i18n:** Escaping of character broke strings at Transifex ([aaa608a](https://github.com/nextcloud/contacts/commit/aaa608a94691366f4a77e55a57cbc7a95abe4188))
* Icon button alignment ([2218eed](https://github.com/nextcloud/contacts/commit/2218eed261c38c5311d744517d3724cd15e3e9bd))
* **icons:** Use outline icons for teams and user groups ([9744791](https://github.com/nextcloud/contacts/commit/97447916ecce6314fcddec0b02478304ce8d325d))
* import controller should return a translated status message ([a49f592](https://github.com/nextcloud/contacts/commit/a49f592b432dbcc8e8b6e24682d9874009e5a68b))
* **ImportController:** fix translation string for imported messages ([3f8e7b6](https://github.com/nextcloud/contacts/commit/3f8e7b69cf16b8ac92fbeb1bb24919c419c37513))
* improve expand button visibility in OrgChart dark mode ([99ad89e](https://github.com/nextcloud/contacts/commit/99ad89eab504a30e30629ad889998e33d648d28d))
* improve matching for tel type parameter ([503ae11](https://github.com/nextcloud/contacts/commit/503ae11e1f23c5d77c7e704cb65f813b43327066))
* Instantiation of ContactsManager ([800f4bd](https://github.com/nextcloud/contacts/commit/800f4bdfd91296d5455ac686b19401bd708de281))
* **l10n:** Fix plural form ([d6acab1](https://github.com/nextcloud/contacts/commit/d6acab1696ace15e840dd73cc903da97469dd0d1))
* **l10n:** Update translations from Transifex ([4962b9a](https://github.com/nextcloud/contacts/commit/4962b9a2207d3906b2260a811abc0bf229992226))
* **l10n:** Update translations from Transifex ([f157be9](https://github.com/nextcloud/contacts/commit/f157be90db4a9bb1e7bcd475e8bf33e2d379b71e))
* **l10n:** Update translations from Transifex ([2c19e9d](https://github.com/nextcloud/contacts/commit/2c19e9d1ba52b46a0ac32b25e26de738d8c4e40a))
* **l10n:** Update translations from Transifex ([0710e3c](https://github.com/nextcloud/contacts/commit/0710e3ccd436498756c347ae32dc44618caf2f03))
* **l10n:** Update translations from Transifex ([f7a41ae](https://github.com/nextcloud/contacts/commit/f7a41aea278c7794350dc2f990b3f9b27f4fd637))
* **l10n:** Update translations from Transifex ([e223be6](https://github.com/nextcloud/contacts/commit/e223be69282eda62c0540c9c88d2e868214204d3))
* **l10n:** Update translations from Transifex ([df1f455](https://github.com/nextcloud/contacts/commit/df1f45535a911dd4c21f6349076d508e38d5c946))
* **l10n:** Update translations from Transifex ([e8be3b6](https://github.com/nextcloud/contacts/commit/e8be3b6b704b1b4bd042f41f91ae375c2de7f36e))
* **l10n:** Update translations from Transifex ([b8d3a73](https://github.com/nextcloud/contacts/commit/b8d3a731d0b4d003c44969df2ec9ffa14c555852))
* **l10n:** Update translations from Transifex ([d7eb2d0](https://github.com/nextcloud/contacts/commit/d7eb2d0866fac225ea2e9e293a1a3dfd6353a7a7))
* **l10n:** Update translations from Transifex ([5ab5629](https://github.com/nextcloud/contacts/commit/5ab5629740c39823955410ab7aebf720b0bac856))
* **l10n:** Update translations from Transifex ([6dd7469](https://github.com/nextcloud/contacts/commit/6dd746949ab355a496c0bb4c87c6fd20da505d4c))
* **l10n:** Update translations from Transifex ([1a7aee2](https://github.com/nextcloud/contacts/commit/1a7aee2583256184a0a9285f1b39b6173b5136aa))
* **l10n:** Update translations from Transifex ([cb22948](https://github.com/nextcloud/contacts/commit/cb229480073afe45d1ea9fa6237711a056661072))
* **l10n:** Update translations from Transifex ([a5f10c1](https://github.com/nextcloud/contacts/commit/a5f10c115a27745c81e7fd1bce6b7c0fc953a8ee))
* **l10n:** Update translations from Transifex ([f46c703](https://github.com/nextcloud/contacts/commit/f46c703c8527936ed18ba8a34077d1fe1962b7f6))
* **l10n:** Update translations from Transifex ([1bd40eb](https://github.com/nextcloud/contacts/commit/1bd40ebc0a196a957c604653fb5648f0b0903653))
* **l10n:** Update translations from Transifex ([071bd72](https://github.com/nextcloud/contacts/commit/071bd72aab5957a3e72ae687f1fca52d07584cf3))
* **l10n:** Update translations from Transifex ([a6b517c](https://github.com/nextcloud/contacts/commit/a6b517c5ca3f41e972b10346adae07b413802a89))
* **l10n:** Update translations from Transifex ([00dd7f0](https://github.com/nextcloud/contacts/commit/00dd7f09c74b5325d673151a0b3fae55362759f9))
* **l10n:** Update translations from Transifex ([a086ba8](https://github.com/nextcloud/contacts/commit/a086ba88fe54b34dc2d7d8b38e6627122ba60e24))
* **l10n:** Update translations from Transifex ([8b7f2d8](https://github.com/nextcloud/contacts/commit/8b7f2d81cdd60ae4ad41b4e6d0102e3321cd5305))
* **l10n:** Update translations from Transifex ([aa8b8a4](https://github.com/nextcloud/contacts/commit/aa8b8a4970c6fbb5a2652992c149be154f6227f9))
* **l10n:** Update translations from Transifex ([4bf7b49](https://github.com/nextcloud/contacts/commit/4bf7b49a1d025eae1ca983b10d1ee94400b6aab1))
* **l10n:** Update translations from Transifex ([142abf1](https://github.com/nextcloud/contacts/commit/142abf1268bfe71b5cedbb286f2536e7f1c04beb))
* **l10n:** Update translations from Transifex ([20605be](https://github.com/nextcloud/contacts/commit/20605beaad0374d8bd3ce6d0a96cae5344d5636e))
* **l10n:** Update translations from Transifex ([9b66ad3](https://github.com/nextcloud/contacts/commit/9b66ad32d7f0d89deb3e78ea3d375359773633f6))
* **l10n:** Update translations from Transifex ([6d8101d](https://github.com/nextcloud/contacts/commit/6d8101d1e41d36ac893686e479b50107b301ebe6))
* **l10n:** Update translations from Transifex ([8fdf9a9](https://github.com/nextcloud/contacts/commit/8fdf9a9072f8197323e1582c5be4e5525897fdcb))
* **l10n:** Update translations from Transifex ([866bdb0](https://github.com/nextcloud/contacts/commit/866bdb01136fd42da63fad180221023deca209f2))
* **l10n:** Update translations from Transifex ([8028a7d](https://github.com/nextcloud/contacts/commit/8028a7d19e02636768c9f3d124c6a64978b6c124))
* **l10n:** Update translations from Transifex ([c21fbbc](https://github.com/nextcloud/contacts/commit/c21fbbc5678837b526c4eaeac09832e095860871))
* **l10n:** Update translations from Transifex ([7eaff84](https://github.com/nextcloud/contacts/commit/7eaff8466aa9a9d9e53a64c678a158777d7bb4c2))
* **l10n:** Update translations from Transifex ([9b9a85b](https://github.com/nextcloud/contacts/commit/9b9a85bddaf2b7d83fd5de24cfac342034a130db))
* **l10n:** Update translations from Transifex ([d0d2007](https://github.com/nextcloud/contacts/commit/d0d2007aa64751c3d48d028546565247eb355c24))
* **l10n:** Update translations from Transifex ([e70b955](https://github.com/nextcloud/contacts/commit/e70b95536c17856d8ccc10bac7917d5f8d7224e3))
* **l10n:** Update translations from Transifex ([81df9af](https://github.com/nextcloud/contacts/commit/81df9af02eee63fe1b99a7843c1c76bd82440ab9))
* **l10n:** Update translations from Transifex ([e4e5426](https://github.com/nextcloud/contacts/commit/e4e5426c0be51e4b56e0f9fb0eea9e83809bf960))
* **l10n:** Update translations from Transifex ([b352fcb](https://github.com/nextcloud/contacts/commit/b352fcbf0da6e3caddba6d79a9edd60a79a3cfbe))
* **l10n:** Update translations from Transifex ([a6a1fbc](https://github.com/nextcloud/contacts/commit/a6a1fbcf53d082eebe14d964e299105970818fe7))
* **l10n:** Update translations from Transifex ([1d7f6f0](https://github.com/nextcloud/contacts/commit/1d7f6f0180bc7ea75ac0c9c7b7dea47f5114f6b1))
* **l10n:** Update translations from Transifex ([d7b7ccd](https://github.com/nextcloud/contacts/commit/d7b7ccd3a3af381b0d8983983d2d18f5dbfdd6fb))
* **l10n:** Update translations from Transifex ([9e10bb8](https://github.com/nextcloud/contacts/commit/9e10bb8791cf7fd750cda26e4b63100e9b1bce3b))
* **l10n:** Update translations from Transifex ([20a040c](https://github.com/nextcloud/contacts/commit/20a040ce4fe69ecc952ec4d92ac4ffc92928ba67))
* **l10n:** Update translations from Transifex ([e149550](https://github.com/nextcloud/contacts/commit/e1495501cf46834e5432144eb94035397c00eba5))
* **l10n:** Update translations from Transifex ([b103581](https://github.com/nextcloud/contacts/commit/b103581ca82a50048e91dc1144c95a8f11b931a0))
* **l10n:** Update translations from Transifex ([7c54c65](https://github.com/nextcloud/contacts/commit/7c54c65cd5b60641ec0e259d401974825a4b4375))
* **l10n:** Update translations from Transifex ([96cfca2](https://github.com/nextcloud/contacts/commit/96cfca26ac34e9603dd873bee9d11aea110345a6))
* **l10n:** Update translations from Transifex ([ee78102](https://github.com/nextcloud/contacts/commit/ee781026553179aadd5678e09b9cb94867dd2436))
* **l10n:** Update translations from Transifex ([8312597](https://github.com/nextcloud/contacts/commit/8312597f78fc3fc4e7da1bbad007bcf39bbbe6bb))
* **l10n:** Update translations from Transifex ([a1e23cb](https://github.com/nextcloud/contacts/commit/a1e23cb927b56890cdd37337017be5c5a2ab4813))
* **l10n:** Update translations from Transifex ([b625357](https://github.com/nextcloud/contacts/commit/b625357ac6a1dc58e590506726e16d40c535a18e))
* **l10n:** Update translations from Transifex ([6fa3d59](https://github.com/nextcloud/contacts/commit/6fa3d59cd7985141c9b42281d2a1ea4c3f6ca640))
* **l10n:** Update translations from Transifex ([f08c1af](https://github.com/nextcloud/contacts/commit/f08c1af336b69a179255a3f37af31b5a3acf8837))
* **l10n:** Update translations from Transifex ([9e028b4](https://github.com/nextcloud/contacts/commit/9e028b43e5fba00d0b3e9b61aac55062ef5bead5))
* **l10n:** Update translations from Transifex ([09c766a](https://github.com/nextcloud/contacts/commit/09c766a25de01418e01a159c57c3b838d7ca68c3))
* **l10n:** Update translations from Transifex ([29a7c43](https://github.com/nextcloud/contacts/commit/29a7c432f3e53c062f96bdf5597cf678a6aac183))
* **l10n:** Update translations from Transifex ([2016bf3](https://github.com/nextcloud/contacts/commit/2016bf3ee605f9953bbf5839e319cbd25362fea7))
* **l10n:** Update translations from Transifex ([bf2fa16](https://github.com/nextcloud/contacts/commit/bf2fa169252094d05d473f8d777c49d2a69bab3f))
* **l10n:** Update translations from Transifex ([4405fe8](https://github.com/nextcloud/contacts/commit/4405fe8662021bd55111ab84d1bb25edde1e830b))
* **l10n:** Update translations from Transifex ([5064ca7](https://github.com/nextcloud/contacts/commit/5064ca7473eae832cd86fa74a3d1866c61605d3c))
* **l10n:** Update translations from Transifex ([fd48b03](https://github.com/nextcloud/contacts/commit/fd48b03c5d1f850fc2657fbd9522c1576dbeb4bc))
* **l10n:** Update translations from Transifex ([083489a](https://github.com/nextcloud/contacts/commit/083489af0590e128dc0580a6db9b6db442fbee8e))
* **l10n:** Update translations from Transifex ([8b34248](https://github.com/nextcloud/contacts/commit/8b34248ccee696d06e0a510738a78a5ab1da0caa))
* **l10n:** Update translations from Transifex ([68df40c](https://github.com/nextcloud/contacts/commit/68df40c00ca5d537d503fb02456720f899a73752))
* **l10n:** Update translations from Transifex ([ae49d56](https://github.com/nextcloud/contacts/commit/ae49d5649c8d67e2ec9dedadfadd2e7220cddaca))
* **l10n:** Update translations from Transifex ([26a4ebf](https://github.com/nextcloud/contacts/commit/26a4ebf3aaa49ac077972bbaae16b19d9aa43753))
* **l10n:** Update translations from Transifex ([cde151f](https://github.com/nextcloud/contacts/commit/cde151ff7812ca6efb6b83376252152e71b24ea1))
* **l10n:** Update translations from Transifex ([4bdd7dd](https://github.com/nextcloud/contacts/commit/4bdd7ddfe6163cda6206c9cb3d51b3df45446fc4))
* **l10n:** Update translations from Transifex ([b0aa7bc](https://github.com/nextcloud/contacts/commit/b0aa7bcf580cd9e403820327fe572bd2cd518cf1))
* **l10n:** Update translations from Transifex ([0c1dd9f](https://github.com/nextcloud/contacts/commit/0c1dd9f2878baeb834aaa04609b328b277f9faba))
* **l10n:** Update translations from Transifex ([4ed7bd8](https://github.com/nextcloud/contacts/commit/4ed7bd8a5bfa64cf7912cd098c757a65f7bc04a5))
* **l10n:** Update translations from Transifex ([22f2a13](https://github.com/nextcloud/contacts/commit/22f2a133dffef59abccb1c179672225b33756046))
* **l10n:** Update translations from Transifex ([8c0510c](https://github.com/nextcloud/contacts/commit/8c0510c792080fc4c815f0d670615003f44dcf84))
* **l10n:** Update translations from Transifex ([8f1925a](https://github.com/nextcloud/contacts/commit/8f1925aff792e156e726860e8f81208647441eea))
* **l10n:** Update translations from Transifex ([1c367cb](https://github.com/nextcloud/contacts/commit/1c367cb3c4ca6ee64a6fdc27ce3283525f34af32))
* **l10n:** Update translations from Transifex ([e7389b7](https://github.com/nextcloud/contacts/commit/e7389b7f286eb6946ab892e0e4e6bfb706208af8))
* **l10n:** Update translations from Transifex ([579315b](https://github.com/nextcloud/contacts/commit/579315be043e9ab4c196e95d80a2617a3c81d286))
* **l10n:** Update translations from Transifex ([9c57b90](https://github.com/nextcloud/contacts/commit/9c57b90aa23311e01e29a8e6561bd478911e8965))
* **l10n:** Update translations from Transifex ([73af31e](https://github.com/nextcloud/contacts/commit/73af31ec7f611b94f1c545c74eed353d37d5fa4b))
* **l10n:** Update translations from Transifex ([cf7cbec](https://github.com/nextcloud/contacts/commit/cf7cbece3a35ebbec1ffc254db23d67e08b3baec))
* **l10n:** Update translations from Transifex ([7987cf6](https://github.com/nextcloud/contacts/commit/7987cf65f077baa0c33cd5a0b5d02679b8269782))
* **l10n:** Update translations from Transifex ([aacf691](https://github.com/nextcloud/contacts/commit/aacf69113a58a508e1eb3bfb4f7407821a05b833))
* **l10n:** Update translations from Transifex ([91b36f8](https://github.com/nextcloud/contacts/commit/91b36f88a5168cdbf80677d4aa620463ac894418))
* **l10n:** Update translations from Transifex ([7a6a33c](https://github.com/nextcloud/contacts/commit/7a6a33cf8e8dfd210713f56f796fe5b79ef09850))
* **l10n:** Update translations from Transifex ([a505a09](https://github.com/nextcloud/contacts/commit/a505a09c67809d3d5b5a2ef955ce6de0629563c9))
* **l10n:** Update translations from Transifex ([dcb6160](https://github.com/nextcloud/contacts/commit/dcb61604a0eafc3d314d3d3d3733884a66acb4a7))
* **l10n:** Update translations from Transifex ([50ffced](https://github.com/nextcloud/contacts/commit/50ffced5a5657f22004fcd9a246e9fc6fa9b2f46))
* **l10n:** Update translations from Transifex ([aef743b](https://github.com/nextcloud/contacts/commit/aef743b75d53b68bf504b8422195055a5d39eeae))
* **l10n:** Update translations from Transifex ([3f8ba46](https://github.com/nextcloud/contacts/commit/3f8ba46570eed2c864be96b7c1ea925c86ef080c))
* **l10n:** Update translations from Transifex ([dfc1ca9](https://github.com/nextcloud/contacts/commit/dfc1ca9345472c73036b19ce6ea414a41ccfa4bf))
* **l10n:** Update translations from Transifex ([c67a2d1](https://github.com/nextcloud/contacts/commit/c67a2d15d7264e173c61208a487e2a173d4a644c))
* **l10n:** Update translations from Transifex ([b5ee336](https://github.com/nextcloud/contacts/commit/b5ee3369ca46ad69803973be8a8c1a72abbe39d7))
* **l10n:** Update translations from Transifex ([141929f](https://github.com/nextcloud/contacts/commit/141929fa11f397dc32c0b5afd4e241dd8644385d))
* **l10n:** Update translations from Transifex ([a3f85ea](https://github.com/nextcloud/contacts/commit/a3f85ea8e95676badf633a9170c8c8bef2f1f203))
* **l10n:** Update translations from Transifex ([e93f9a6](https://github.com/nextcloud/contacts/commit/e93f9a64db21d88a8a40522daad972ae9a5df765))
* **l10n:** Update translations from Transifex ([b977fd8](https://github.com/nextcloud/contacts/commit/b977fd88d5e7c51f3da31c0403d1db7d02aff9e8))
* **l10n:** Update translations from Transifex ([1681df1](https://github.com/nextcloud/contacts/commit/1681df11f8c41b85d35a3a9852cb5762b19d8ea8))
* **l10n:** Update translations from Transifex ([efa65ac](https://github.com/nextcloud/contacts/commit/efa65ac2ca896b606ca0850b5d1920fb9310b20b))
* **l10n:** Update translations from Transifex ([6972fe9](https://github.com/nextcloud/contacts/commit/6972fe9849bc5c2eafaa1a2c4849c2bc438140c1))
* **l10n:** Update translations from Transifex ([e4d0c82](https://github.com/nextcloud/contacts/commit/e4d0c823c760821d563b7ed3aa101d9bbb573ee8))
* **l10n:** Update translations from Transifex ([8478d48](https://github.com/nextcloud/contacts/commit/8478d4860396184a3083f94f9daf6b8b49a9f366))
* **l10n:** Update translations from Transifex ([6219c9d](https://github.com/nextcloud/contacts/commit/6219c9db36f0b7b4a8b71e8d6f54f0a348aca4c7))
* **l10n:** Update translations from Transifex ([1ff5b6c](https://github.com/nextcloud/contacts/commit/1ff5b6c2f87c2aaef9cf64c948e9ce76097480cd))
* **l10n:** Update translations from Transifex ([b6db3e5](https://github.com/nextcloud/contacts/commit/b6db3e56f5652479f7dfd63c9a11576a4ae1e9bc))
* **l10n:** Update translations from Transifex ([a7ceafb](https://github.com/nextcloud/contacts/commit/a7ceafb7a44c60282bf1d29c9f562d21421d6ae7))
* **l10n:** Update translations from Transifex ([5451e5a](https://github.com/nextcloud/contacts/commit/5451e5ae3c795f15e9a88df4a0e15308c0805945))
* **l10n:** Update translations from Transifex ([45675a6](https://github.com/nextcloud/contacts/commit/45675a696182ae82a330c3f73ecb2aaa96a0f806))
* **l10n:** Update translations from Transifex ([af1237e](https://github.com/nextcloud/contacts/commit/af1237edf1c3163216850254d5489cc1403f9b3b))
* **l10n:** Update translations from Transifex ([91359e8](https://github.com/nextcloud/contacts/commit/91359e8ab091eba05a9989b06b1189aeaa5c5579))
* **l10n:** Update translations from Transifex ([428d973](https://github.com/nextcloud/contacts/commit/428d973a9a896b768cb8294da9f2a2dff7de5e73))
* **l10n:** Update translations from Transifex ([e1c667c](https://github.com/nextcloud/contacts/commit/e1c667c2dd30885e76512a847db50883a75b1687))
* **l10n:** Update translations from Transifex ([2e24f29](https://github.com/nextcloud/contacts/commit/2e24f294e7446398931b9b36641d0fb91f925ac3))
* **l10n:** Update translations from Transifex ([8544627](https://github.com/nextcloud/contacts/commit/854462722d37f3525333f09572070b0cbc2d441c))
* **l10n:** Update translations from Transifex ([fa2214c](https://github.com/nextcloud/contacts/commit/fa2214c17dc93b38f774364a0b4cc421df480864))
* **l10n:** Update translations from Transifex ([d666eb2](https://github.com/nextcloud/contacts/commit/d666eb2122bad174019cb83363f13f35c8b75cab))
* **l10n:** Update translations from Transifex ([d274d7f](https://github.com/nextcloud/contacts/commit/d274d7f1818b5d9bbe87b3b746b218cff033b7c9))
* **l10n:** Update translations from Transifex ([a3cd2c5](https://github.com/nextcloud/contacts/commit/a3cd2c55a4b2471e1dbe3fa48d8b982717e1ef96))
* **l10n:** Update translations from Transifex ([ecc4b33](https://github.com/nextcloud/contacts/commit/ecc4b336da59f1799a09436f1940373173767bc6))
* **l10n:** Update translations from Transifex ([fac8d2f](https://github.com/nextcloud/contacts/commit/fac8d2f1e8c0fbe446cc8b9f5a2512b029e76bf9))
* **l10n:** Update translations from Transifex ([c98a2da](https://github.com/nextcloud/contacts/commit/c98a2dabe0511830c17a77830783e03b5dc12b04))
* **l10n:** Update translations from Transifex ([72e3606](https://github.com/nextcloud/contacts/commit/72e36063e84d850217d6d97ddda3e7d32fae94f7))
* **l10n:** Update translations from Transifex ([1bdcd79](https://github.com/nextcloud/contacts/commit/1bdcd79c672cd04b2313d77845d60780887ffe88))
* **l10n:** Update translations from Transifex ([7f5da02](https://github.com/nextcloud/contacts/commit/7f5da02c9125669b78c9a98c5864a3af14446c77))
* **l10n:** Update translations from Transifex ([1c31107](https://github.com/nextcloud/contacts/commit/1c31107542351d65ce6799b25a59d0ca52f41318))
* **l10n:** Update translations from Transifex ([9c9b007](https://github.com/nextcloud/contacts/commit/9c9b007ec3602b63f85e691676e43d1cf3a22ee6))
* **l10n:** Update translations from Transifex ([4841f20](https://github.com/nextcloud/contacts/commit/4841f202e14ff3a032daeea25118877b2fdaa15c))
* **l10n:** Update translations from Transifex ([6967ae4](https://github.com/nextcloud/contacts/commit/6967ae45ee03aa3b1e8ee2f1c21476450b34a968))
* **l10n:** Update translations from Transifex ([ca7e374](https://github.com/nextcloud/contacts/commit/ca7e374e5e8b385c62bc59b77e48a305018219c4))
* **l10n:** Update translations from Transifex ([b3318c3](https://github.com/nextcloud/contacts/commit/b3318c3c1d82a52ceb8fb5aaab98865a1ee08a31))
* **l10n:** Update translations from Transifex ([ef2dbd7](https://github.com/nextcloud/contacts/commit/ef2dbd7d712a27683ca40615dc6b70a2778087d3))
* **l10n:** Update translations from Transifex ([2491e2b](https://github.com/nextcloud/contacts/commit/2491e2bf1c11bf3f536dbc20aed206faa3d2431f))
* **l10n:** Update translations from Transifex ([fcec8cc](https://github.com/nextcloud/contacts/commit/fcec8cce4b3ce1dda5bf06417e3f7fb0ed5e56cf))
* **l10n:** Update translations from Transifex ([77d5497](https://github.com/nextcloud/contacts/commit/77d5497d46905aeb9c67e5af188aeb4ad9520c30))
* **l10n:** Update translations from Transifex ([487b023](https://github.com/nextcloud/contacts/commit/487b023c65feb81716b056ed91ca192bf66b8320))
* **l10n:** Update translations from Transifex ([66f0ea1](https://github.com/nextcloud/contacts/commit/66f0ea1fa0c4a66a5cd74e25763ba04e578d6b39))
* **l10n:** Update translations from Transifex ([f48891e](https://github.com/nextcloud/contacts/commit/f48891e3052f474e57bd568ac5d8fe53a0a65788))
* **l10n:** Update translations from Transifex ([39d578e](https://github.com/nextcloud/contacts/commit/39d578e2d5b58f4944c5b69beedcae1916606bcd))
* **l10n:** Update translations from Transifex ([5d7285c](https://github.com/nextcloud/contacts/commit/5d7285c746f58ded5e018ab34b66d29c117548a5))
* **l10n:** Update translations from Transifex ([4334ec7](https://github.com/nextcloud/contacts/commit/4334ec7f1db9f256fdd6fbbf5181833af7d483da))
* **l10n:** Update translations from Transifex ([7a9c18c](https://github.com/nextcloud/contacts/commit/7a9c18c4d657c2126c6390d9836f6d38d97243b7))
* **l10n:** Update translations from Transifex ([ad363d5](https://github.com/nextcloud/contacts/commit/ad363d5d77407a1b934e9dab7e1beba488cd73d3))
* **l10n:** Update translations from Transifex ([f67a9b5](https://github.com/nextcloud/contacts/commit/f67a9b56f4feed3bf11793a73c05ad05a399910b))
* **l10n:** Update translations from Transifex ([bf94c7c](https://github.com/nextcloud/contacts/commit/bf94c7c095b1efa4c3c9c769adf8e4f079cf4ddc))
* **l10n:** Update translations from Transifex ([bf5136b](https://github.com/nextcloud/contacts/commit/bf5136b4721bf4bb40d570ac355e6117abfeea6b))
* **l10n:** Update translations from Transifex ([38aef0c](https://github.com/nextcloud/contacts/commit/38aef0caaa6f78a13aef04336efd8ab1983e5712))
* **l10n:** Update translations from Transifex ([2f43d75](https://github.com/nextcloud/contacts/commit/2f43d75c9e80e762681d7b0ec5abbb091ded49e9))
* **l10n:** Update translations from Transifex ([69b5030](https://github.com/nextcloud/contacts/commit/69b5030302fea6d5913f8ef06070b2f0bdca4d3e))
* **l10n:** Update translations from Transifex ([7bd3309](https://github.com/nextcloud/contacts/commit/7bd330948d91c470da852f2a7090ed0eb5f5be7f))
* **l10n:** Update translations from Transifex ([45bd927](https://github.com/nextcloud/contacts/commit/45bd92789c310e2e6f691551b8b5bb7fb4524bcd))
* **l10n:** Update translations from Transifex ([c12b745](https://github.com/nextcloud/contacts/commit/c12b74530449ef11d6402df9789ac99538b3a9b1))
* **l10n:** Update translations from Transifex ([15e9313](https://github.com/nextcloud/contacts/commit/15e9313d6dec1dd53cbd535e6f2ecd0c4ec6ceaf))
* **l10n:** Update translations from Transifex ([ad1890b](https://github.com/nextcloud/contacts/commit/ad1890bc8fc0747a68b71c03bb235cde7f689ff0))
* **l10n:** Update translations from Transifex ([b83a356](https://github.com/nextcloud/contacts/commit/b83a356a106f44ed959d332fbb9ed43185f5131d))
* **l10n:** Update translations from Transifex ([b46db45](https://github.com/nextcloud/contacts/commit/b46db45d690da3a4d2c2a8945870e1b5367038fb))
* **l10n:** Update translations from Transifex ([9d34e54](https://github.com/nextcloud/contacts/commit/9d34e549b804c374d5499c7c28d75268a3601a2d))
* **l10n:** Update translations from Transifex ([3cf84c8](https://github.com/nextcloud/contacts/commit/3cf84c839fb5940a5b2dff302194dd0ba059fea2))
* **l10n:** Update translations from Transifex ([d07b1d7](https://github.com/nextcloud/contacts/commit/d07b1d75541925bccb9afc9109a42013d3d4a1be))
* **l10n:** Update translations from Transifex ([27d4b89](https://github.com/nextcloud/contacts/commit/27d4b896cde01a140164c2422d91355b9bae14e4))
* **l10n:** Update translations from Transifex ([fc4496b](https://github.com/nextcloud/contacts/commit/fc4496b8c60958c05d7864ec149ca02d4f80c364))
* **l10n:** Update translations from Transifex ([8dfe46a](https://github.com/nextcloud/contacts/commit/8dfe46a9a6fbd5d3fdf0c2258600f60f786ca662))
* **l10n:** Update translations from Transifex ([ee1aeec](https://github.com/nextcloud/contacts/commit/ee1aeecc939466b0dd220478dcc3cb8d261a0370))
* **l10n:** Update translations from Transifex ([96671b8](https://github.com/nextcloud/contacts/commit/96671b806471452c6b027ab63581e0baea613187))
* **l10n:** Update translations from Transifex ([b66788d](https://github.com/nextcloud/contacts/commit/b66788d6fbf72e8ed03345c403f4dd641bc31d6b))
* **l10n:** Update translations from Transifex ([2012c28](https://github.com/nextcloud/contacts/commit/2012c28591f201c9af68e06ac2328c79a4808b4f))
* **l10n:** Update translations from Transifex ([713c7cf](https://github.com/nextcloud/contacts/commit/713c7cfbe7b91833c14d3e761b3d93d31f29abf2))
* **l10n:** Update translations from Transifex ([1a818fb](https://github.com/nextcloud/contacts/commit/1a818fb75cbcea8104465b6db80af95f79f6610c))
* **l10n:** Update translations from Transifex ([5370c2e](https://github.com/nextcloud/contacts/commit/5370c2e0aedd9ee878f9d3fca1962d3c4971a324))
* **l10n:** Update translations from Transifex ([e9b4fbd](https://github.com/nextcloud/contacts/commit/e9b4fbd6b14dd97e3807980cdaa94d145eb5244d))
* **l10n:** Update translations from Transifex ([a85efeb](https://github.com/nextcloud/contacts/commit/a85efeba20acc58b19e68f7fd6b3b52a5a320c15))
* **l10n:** Update translations from Transifex ([19fb787](https://github.com/nextcloud/contacts/commit/19fb787dd14ccbf58b8c6f72df315abe6bcbf58a))
* **l10n:** Update translations from Transifex ([4f47f6c](https://github.com/nextcloud/contacts/commit/4f47f6c75bb03a3add5b44d2d990844de8667b2f))
* **l10n:** Update translations from Transifex ([ddf4a49](https://github.com/nextcloud/contacts/commit/ddf4a49d450e636c699feaec711585dfd3b473a5))
* **l10n:** Update translations from Transifex ([d22489b](https://github.com/nextcloud/contacts/commit/d22489ba3f5c2c75a240a127cf2e0839bd6ab21f))
* **l10n:** Update translations from Transifex ([e8fce52](https://github.com/nextcloud/contacts/commit/e8fce529bb07a164fcc0aac44d59ecd86a6d06ea))
* **l10n:** Update translations from Transifex ([f011570](https://github.com/nextcloud/contacts/commit/f011570c84202ace2d59b56b24bcf66d369f026c))
* **l10n:** Update translations from Transifex ([63b3872](https://github.com/nextcloud/contacts/commit/63b38724039d53fa756775d8510d05e8a02e34dd))
* **l10n:** Update translations from Transifex ([a54c58a](https://github.com/nextcloud/contacts/commit/a54c58a47fd731b9be7029412eba159b392e70af))
* **l10n:** Update translations from Transifex ([7eb5d0f](https://github.com/nextcloud/contacts/commit/7eb5d0fecbe50b842d3865a7e106c4edf1d9ca02))
* **l10n:** Update translations from Transifex ([7ac5bf6](https://github.com/nextcloud/contacts/commit/7ac5bf60a30126b8fc3d7a6c506a8f6f5ae776ef))
* **l10n:** Update translations from Transifex ([ddba290](https://github.com/nextcloud/contacts/commit/ddba290f786e1ee6f5ba05e1640640e6eb9a6788))
* **l10n:** Update translations from Transifex ([c7c67da](https://github.com/nextcloud/contacts/commit/c7c67da0066e575762e080f52770bcaacac5b1f0))
* **l10n:** Update translations from Transifex ([0d4f95e](https://github.com/nextcloud/contacts/commit/0d4f95eee3fc442337ca20e58ed510a00857044e))
* **l10n:** Update translations from Transifex ([10d4cfe](https://github.com/nextcloud/contacts/commit/10d4cfedbcc506b481dc8abc95ac7567cfc33bb4))
* **l10n:** Update translations from Transifex ([aec6949](https://github.com/nextcloud/contacts/commit/aec69496d7b18e9420a717c8838d61c72266f963))
* **l10n:** Update translations from Transifex ([a0a2fb8](https://github.com/nextcloud/contacts/commit/a0a2fb88503df7bfa9c66ee32a4a0300986fd4b6))
* **l10n:** Update translations from Transifex ([5bb2d43](https://github.com/nextcloud/contacts/commit/5bb2d4365720b62318f0c6d5c56fcdf226f7ce36))
* **l10n:** Update translations from Transifex ([081a6b0](https://github.com/nextcloud/contacts/commit/081a6b0f491c717f1fa7c63aa4aeb6718ce639fa))
* **l10n:** Update translations from Transifex ([a113351](https://github.com/nextcloud/contacts/commit/a11335125599bb17eba8ed97eb02454907b4e5ac))
* **l10n:** Update translations from Transifex ([fa6b898](https://github.com/nextcloud/contacts/commit/fa6b898bd904c0a8450fb9543ebba0eeb90cd6a3))
* **l10n:** Update translations from Transifex ([4ad2703](https://github.com/nextcloud/contacts/commit/4ad2703ad9e61176590c35e3fb707d5fe537e9d4))
* **l10n:** Update translations from Transifex ([0e9ae91](https://github.com/nextcloud/contacts/commit/0e9ae915bc5909b0b8b5fb9926861472115c8bb9))
* **l10n:** Update translations from Transifex ([543b6bf](https://github.com/nextcloud/contacts/commit/543b6bf76d9ccdeafd7a53a41aff386f89403b5e))
* **l10n:** Update translations from Transifex ([48e50db](https://github.com/nextcloud/contacts/commit/48e50dbe528232192d500c4495f8dceb8a63bbcf))
* **l10n:** Update translations from Transifex ([33a5d99](https://github.com/nextcloud/contacts/commit/33a5d99441b6743a04ca82da8fd4fc4c856d7685))
* **l10n:** Update translations from Transifex ([3ead8eb](https://github.com/nextcloud/contacts/commit/3ead8eb8b5cb2d696f83bd9ed9e918d3ab83f343))
* **l10n:** Update translations from Transifex ([43115c2](https://github.com/nextcloud/contacts/commit/43115c268be30e3d6d6af268988e49d4814f3a35))
* **l10n:** Update translations from Transifex ([76b2e72](https://github.com/nextcloud/contacts/commit/76b2e720c68bca2879c0b795b36bb53141c098a5))
* **l10n:** Update translations from Transifex ([caab18e](https://github.com/nextcloud/contacts/commit/caab18ee063ba3534b78f0f08b460a0966f59376))
* **l10n:** Update translations from Transifex ([13b69e7](https://github.com/nextcloud/contacts/commit/13b69e7b3c2c3d3becd3f6f6209c642ef73f7be1))
* **l10n:** Update translations from Transifex ([a7b347e](https://github.com/nextcloud/contacts/commit/a7b347eb9b30581e387ec3740d74b1b1e21575d1))
* **l10n:** Update translations from Transifex ([852739b](https://github.com/nextcloud/contacts/commit/852739bb389e44d1a369f37e7950878eba658733))
* **l10n:** Update translations from Transifex ([cf5ae3e](https://github.com/nextcloud/contacts/commit/cf5ae3e476dad72bdab8bfdb87f8452e198c6d21))
* **l10n:** Update translations from Transifex ([0714ac0](https://github.com/nextcloud/contacts/commit/0714ac049c8e272c33c2544451fa1569d20a7e54))
* **l10n:** Update translations from Transifex ([1d4ae6e](https://github.com/nextcloud/contacts/commit/1d4ae6edca44802e43d148548765c89ffd32b1ec))
* **l10n:** Update translations from Transifex ([9456773](https://github.com/nextcloud/contacts/commit/9456773f28636b6dc22680c89dcabd76755ed490))
* **l10n:** Update translations from Transifex ([7260513](https://github.com/nextcloud/contacts/commit/7260513c8b72cf0e76cc162e3a8360bcad27008a))
* **l10n:** Update translations from Transifex ([5dbac3d](https://github.com/nextcloud/contacts/commit/5dbac3d0778f1fb1fc56f9a5f1a90cef4af812fb))
* lint error ([90fcf1c](https://github.com/nextcloud/contacts/commit/90fcf1cf3f87f9dc4a896d0339b7ed9324519a61))
* **lint-php-cs:** use minimum available php version ([650eebf](https://github.com/nextcloud/contacts/commit/650eebf91854dc8b671120b176023bb4b81cf500))
* log error while saving contact ([d3ac0ed](https://github.com/nextcloud/contacts/commit/d3ac0ed9f6f633a6857bfc261a7c283673004d09))
* long contact name overlapping with other content ([1db37fb](https://github.com/nextcloud/contacts/commit/1db37fb0e3a51d9dd046be576723f2ed9891659e))
* Make circle members visible again by moving them out of the modal ([f25e5d1](https://github.com/nextcloud/contacts/commit/f25e5d13271346d13b76a40c4418b095d6e2eab2))
* **MemberList:** Don't use VirtualList for listing members ([3bf8e6c](https://github.com/nextcloud/contacts/commit/3bf8e6c797fddcc836ef5a8dbfb81373185d7ddb)), closes [#3996](https://github.com/nextcloud/contacts/issues/3996)
* **MembersListItem:** Adjustments to ListItemIcon properties ([5bb226f](https://github.com/nextcloud/contacts/commit/5bb226f36aa538f98424341b7b5fa8129d277c20))
* **Members:** Rename 'Contact groups' to 'groups' ([2d98c29](https://github.com/nextcloud/contacts/commit/2d98c296f8da282f340b4afaed157ae25f6fde49)), closes [#3607](https://github.com/nextcloud/contacts/issues/3607)
* **Merging:** bdays weren't working and misalignments ([fc2895f](https://github.com/nextcloud/contacts/commit/fc2895fb155a79c6a9b7c09fffd7d3dff252c82b))
* missing getLocale import ([968cc6c](https://github.com/nextcloud/contacts/commit/968cc6c1caf1a23b496d4f583b235e6c9710724c))
* **orgchart:** revive node highlighting ([5f2f6d7](https://github.com/nextcloud/contacts/commit/5f2f6d7132db865035e5779786b44f83b88e0fab))
* pre fetch note field to enable searching ([fea656e](https://github.com/nextcloud/contacts/commit/fea656eb7eaec827645457737750323f820138e4))
* **PropertyRev:** make text have accessible contrast ([c219f8e](https://github.com/nextcloud/contacts/commit/c219f8ef2e567b7e786bb83332a3230e0b61a4f0))
* **PropertyText,PropertyMultipleText:** make readonly inputs p for proper wrapping and a11y ([9a1ae13](https://github.com/nextcloud/contacts/commit/9a1ae13f6f7087a78016085e75b2e245195e1774))
* **PropertyText:** textarea auto resizing ([5c6a510](https://github.com/nextcloud/contacts/commit/5c6a510181010a46df3e708c900992ab1369804e))
* **PropertyTitle:** fix property undefined ([06d43e5](https://github.com/nextcloud/contacts/commit/06d43e5c70e15f80ae514d7b027c3175f00d756d))
* **proprety-text:** only resize text area when the ref is defined ([6a067eb](https://github.com/nextcloud/contacts/commit/6a067ebf3660a2d00ae63d1b4f8e87229f541d57))
* **ReadOnlyContactDetails:** remove box shadow ([87be2f4](https://github.com/nextcloud/contacts/commit/87be2f4e772d9008fd77a872ec8ae7b588f8f435))
* redesign seetings toggle to Nextcloud 30 style ([3e2e7e3](https://github.com/nextcloud/contacts/commit/3e2e7e37639c5a6a13dd40d2b794a917a387ef9c))
* reference error in property text ([d0b5100](https://github.com/nextcloud/contacts/commit/d0b5100d4c8b057b815ff1dedd13bb37a0e5325d))
* **release:** Fix wget output option ([75c3b5b](https://github.com/nextcloud/contacts/commit/75c3b5b94038ebd87d9cb583a2f94a93d01a3b17))
* remove app-content-details from global styling ([12707c4](https://github.com/nextcloud/contacts/commit/12707c44de901e44950cb37df6cc3fd1f1f17303))
* remove console log ([4f17fdc](https://github.com/nextcloud/contacts/commit/4f17fdcd68c47074d3327e8dfaf1c23c52fc1c07))
* remove loading contacts-index style ([eca822f](https://github.com/nextcloud/contacts/commit/eca822f6850254d7122b7588c572c101d6e11fd8))
* remove prop mutation ([cec394e](https://github.com/nextcloud/contacts/commit/cec394e8e6b9d4fab324d0a63b94ba768928af3f))
* remove unused style ([2f0282a](https://github.com/nextcloud/contacts/commit/2f0282a354dad9b49165eb635337f9b9cd7960e3))
* rename circle to team for clarity ([5e2f8b7](https://github.com/nextcloud/contacts/commit/5e2f8b76ba4a901ce8186dccddc8634752d13eee))
* rename feature circles to feature teams ([ebfe7fe](https://github.com/nextcloud/contacts/commit/ebfe7fe5fbb339fa8887fd18de2c66c3e2c4bb6b))
* rendering of phone numbers ([fba5432](https://github.com/nextcloud/contacts/commit/fba54325dff0a6d7ebc235de14d67de55eefe930)), closes [/github.com/nextcloud/contacts/pull/1393#issuecomment-570945735](https://github.com//github.com/nextcloud/contacts/pull/1393/issues/issuecomment-570945735)
* resource url ([35ea754](https://github.com/nextcloud/contacts/commit/35ea754c9a23549c0218b8716404873173d46e64))
* respect advanced group sharing settings in frontend ([5450606](https://github.com/nextcloud/contacts/commit/5450606f4832da9e0c60168f827d287423832dfd))
* responsivness for contacts details ([71d3bc9](https://github.com/nextcloud/contacts/commit/71d3bc972b102e43fde5495420928eaf68d04e11))
* restore breaking changes of OCA.Contacts.mountContactDetails ([84233f8](https://github.com/nextcloud/contacts/commit/84233f85a1cea696eca05fcc55b6bc184505337c))
* rev timestamp on contact update ([6a0bb38](https://github.com/nextcloud/contacts/commit/6a0bb38a05be95c8f4ecfd317bf1b92711015f6b))
* Revert "feat: add cancel button for contact editing" ([1e7c21a](https://github.com/nextcloud/contacts/commit/1e7c21a51beecd2e63215864343d7eaf109425e7))
* routing based on base64 encoding ([37f4b12](https://github.com/nextcloud/contacts/commit/37f4b1256d65cdd4234f8d4041e36d00700db1f6))
* **Routing:** add proper routing for circles ([a0a80c9](https://github.com/nextcloud/contacts/commit/a0a80c9f11cd8fb934699ffd92ac9d822e115eaf))
* scrobbale button in navigation ([195a58e](https://github.com/nextcloud/contacts/commit/195a58e74f8f7ba03888ef15abc12f6630d45c5b))
* search should have a way to clear contents ([54e8f32](https://github.com/nextcloud/contacts/commit/54e8f32744a9574290bbd5f6510166daa0987d10))
* **SettingsAddressbook:** prevent sharing an addressbook that has been shared with you ([0baba7a](https://github.com/nextcloud/contacts/commit/0baba7ab6b1ae502df5e41c5ed1d8a674cbf0320))
* **settings:** clarify settings option for open circle ([0aba545](https://github.com/nextcloud/contacts/commit/0aba5455be6362c1e5e5306a4c71cab34097ec28))
* **SettingsSortContacts:** update it to latest NcSelect ([994ceaf](https://github.com/nextcloud/contacts/commit/994ceafe69752bb0b9a5b29eea50b941408ea5dc))
* share address books with user groups ([6e87aca](https://github.com/nextcloud/contacts/commit/6e87aca3c4406cebc4b0699a489bd247355c91b3))
* **sharing:** fix ShareAPI disabled for group list JSON deserialization to array ([97ce69d](https://github.com/nextcloud/contacts/commit/97ce69decfa9aec30c5cc910ccfa258a54d5fc68))
* show address book in read only mode ([249654f](https://github.com/nextcloud/contacts/commit/249654f2336f6fe723508ba3483b2c8208c978b7))
* show error on empty group name ([92aa49c](https://github.com/nextcloud/contacts/commit/92aa49c551658384cfd59fece2d3b1635bd0bead))
* Slot name from title to name for UserBubble ([e4e4939](https://github.com/nextcloud/contacts/commit/e4e4939ee7335f538e500a07a3275cf229580ef3))
* **socialApi:** check avatar mimetype againt allow list ([e0215ed](https://github.com/nextcloud/contacts/commit/e0215ed5520c914e89e55b7ec287a6f78c63ecbd))
* **socialavatar:** Match base class argument name ([b0abe36](https://github.com/nextcloud/contacts/commit/b0abe36e461da17a550f119b4a11c18e7a6c290e))
* **socialavatars:** Fix HTTP client usage ([687ab6b](https://github.com/nextcloud/contacts/commit/687ab6b81d61d6a8cfa278d01f491df21ed6eb03))
* specify origin of request when search for users ([aaa9fc5](https://github.com/nextcloud/contacts/commit/aaa9fc586804fa461447599e3a2af229647f251a))
* **styleint:** apply rtl fixes ([8590f5a](https://github.com/nextcloud/contacts/commit/8590f5ae40b1fbc47e09d818c05d0a8c48d7b183))
* support for RFC 6474 ([b4e205d](https://github.com/nextcloud/contacts/commit/b4e205d946cec2c88eb4d1541c4295c82223a463))
* **teams:** always use populationInherited for population count and update count on population change ([b59a6f0](https://github.com/nextcloud/contacts/commit/b59a6f010211042d1588ca0be99b900a461dab7e))
* **teams:** Make team resources scrollable ([b43c524](https://github.com/nextcloud/contacts/commit/b43c52456390724f7975d3a702c2571dfc9b61f2))
* **teams:** remove results from remote servers when federation is not enabled for the team ([e15dfc8](https://github.com/nextcloud/contacts/commit/e15dfc83d10e0130afad90a6e96dc4ac34875c33))
* **teams:** remove results from untrusted remote servers when searching for members to add ([e8f3acb](https://github.com/nextcloud/contacts/commit/e8f3acbe50583488c291449d1762f6313789735f))
* **teams:** replace 'Team folder' create button with 'Folder' and add notice text ([b37eb3e](https://github.com/nextcloud/contacts/commit/b37eb3e0d90a48e25e7153e4df75107ed6ba5cb5))
* **teams:** Restore accept/reject requests in new ui ([19fc210](https://github.com/nextcloud/contacts/commit/19fc210198c3303b10a055bad2ba57e7fe43b30c))
* **teams:** Restore member management in new ui ([4d0bb51](https://github.com/nextcloud/contacts/commit/4d0bb51eb5fc1380a034fd1136c7b0b1ee7cb46a))
* **teams:** Save team name and description on Save button click ([2e6aa13](https://github.com/nextcloud/contacts/commit/2e6aa13812b5761faa62acbf490a4199be8bf093))
* **teams:** support federated users search when adding team members ([e2b382f](https://github.com/nextcloud/contacts/commit/e2b382f680cac30067a0b5756ddf3c1e1edd517c))
* translation for recently contacted not working ([890d97d](https://github.com/nextcloud/contacts/commit/890d97d82aee445f94b8857cabfc30b36b58f29a))
* **ui:** escape organization/title properties ([3644ed4](https://github.com/nextcloud/contacts/commit/3644ed45c51d4b12a30b74a25b30e62be880f3d1))
* update icon work ([b224e5e](https://github.com/nextcloud/contacts/commit/b224e5eebaf0a3abf9c40f386d18b8b258267237))
* update store state after addressbook change ([d353697](https://github.com/nextcloud/contacts/commit/d353697331c879dd5551305cc590c21b1e0669ec))
* use @nextcloud/momentjs for automatic locale detection/import ([044d46f](https://github.com/nextcloud/contacts/commit/044d46f0f4810cfd8ed29870a272b4e7f7d78e11))
* use built now function instead of manually creating date ([d255c26](https://github.com/nextcloud/contacts/commit/d255c26f0e9ce4e8a2c34d9eb8fe695cd6c8dd16))
* use correct user avatars ([6c161ca](https://github.com/nextcloud/contacts/commit/6c161ca103b3cac64d0e148b37a3da9aa1598dbd))
* use untranslated category for grouping contacts ([f7c0c7d](https://github.com/nextcloud/contacts/commit/f7c0c7da2361b0b20d8bce1fda20fde6cffcd350))
* validate group name ([eb4ea50](https://github.com/nextcloud/contacts/commit/eb4ea50c8dfb49fb7e444427776ae244f24e2510))
* virtual contacts list scroller logic ([54a3b4e](https://github.com/nextcloud/contacts/commit/54a3b4ea2fce78e4cac7c78eb7bf01850ae30769))
* wrong address book displayed on details view ([ed71e14](https://github.com/nextcloud/contacts/commit/ed71e149268400e9a2f346c665940fd1c38066e0))


### Features

* add anniversary, change title ([ca8dfca](https://github.com/nextcloud/contacts/commit/ca8dfca03063db9ec5cd03395fb5cfbbee3bca8e))
* add cancel button for contact editing ([a2d4b5e](https://github.com/nextcloud/contacts/commit/a2d4b5e36a565ebc51e60f715569950f1bd4953b))
* add support for avif contact avatars ([62f3060](https://github.com/nextcloud/contacts/commit/62f30602961bd8178cff9247fce2792b7629df56))
* add support for nextcloud 32 ([a67d356](https://github.com/nextcloud/contacts/commit/a67d35685c10202c2da56a8fa9e9ed5ff1a84136))
* add support for webp contact avatars ([e41a2e1](https://github.com/nextcloud/contacts/commit/e41a2e1cd347eb6e417ae508aafd7021f1b56920))
* adds button to copy values ([8a6360b](https://github.com/nextcloud/contacts/commit/8a6360be51e1fb88b69ffe766120c52afbfbfd31))
* adds edit profile button on own contact ([d3e8baa](https://github.com/nextcloud/contacts/commit/d3e8baae0ca2123674f308d5e41d851467d6f26c))
* Centralize avatar with header ([1f030be](https://github.com/nextcloud/contacts/commit/1f030be850b59de2b1286c21afc4c7bfbb41c8f2))
* **circles:** sort by initiator level and change icons ([143edd6](https://github.com/nextcloud/contacts/commit/143edd6c00b644706574ede20b669f613de62910))
* **ContactsList:** add contact merging action ([025472c](https://github.com/nextcloud/contacts/commit/025472cafe72605d5a511e6a06370dffaf0181ac))
* **ContactsList:** add multiaction for batch adding to group and changing addressbook ([6884678](https://github.com/nextcloud/contacts/commit/6884678784c537ce16a9861a5185df6019ef4a76))
* **ContactsList:** add multiselect action and batch deleting ([d1215ad](https://github.com/nextcloud/contacts/commit/d1215ad7e61d39d52a9a64338bbb7f35cd622ff2))
* **ContactsList:** add multiselect action and batch deleting ([e1639fd](https://github.com/nextcloud/contacts/commit/e1639fdbc258edc12371442e99b6ca5572d8b994))
* **contacts:** Show address book description if provided ([69d14aa](https://github.com/nextcloud/contacts/commit/69d14aa496563295886b0ab5b828dc9a24dd3106))
* create a readonly contactdetails ([b5458b1](https://github.com/nextcloud/contacts/commit/b5458b1caa4c7e18df8a5a359e72fafe359092df))
* create and add guests to teams ([0d9d1e1](https://github.com/nextcloud/contacts/commit/0d9d1e14ddaaf980ae0bfc27a1c577016bd8d15b))
* **deps:** Add Nextcloud 29 support ([cd6b367](https://github.com/nextcloud/contacts/commit/cd6b367783e6b98539420139e1fb8272eddd1ecb))
* **deps:** Add nextcloud 30 support ([425a906](https://github.com/nextcloud/contacts/commit/425a906c101027f489af607b78b4ed35fb86ac5e))
* **deps:** Add Nextcloud 31 support ([166c6ff](https://github.com/nextcloud/contacts/commit/166c6fffff1b33029506d9f61130fd6847a82752))
* **deps:** Add Nextcloud 32 support ([2605739](https://github.com/nextcloud/contacts/commit/2605739e39b836ceca6fcad0dce947232c6325fe))
* **deps:** Add Nextcloud 33 support ([012be7c](https://github.com/nextcloud/contacts/commit/012be7ce30756565bb30bd5f4fad59cddf4129e6))
* **deps:** Add Nextcloud 34 support ([cafae75](https://github.com/nextcloud/contacts/commit/cafae7533fc79dddedb2729afa63196312c10864))
* disable save button on unvalid email address ([896f3a6](https://github.com/nextcloud/contacts/commit/896f3a6b472d093c190437089115b6bdbf2a681f))
* Implement team overview page updates ([e947bcc](https://github.com/nextcloud/contacts/commit/e947bccda653851ca999ba980143c1290e162f43))
* import contacts from files via ocs ([8bb3129](https://github.com/nextcloud/contacts/commit/8bb31298a5fc092ff7202a1d48bf8f987687fd6e))
* improve group adding ([ed0e80e](https://github.com/nextcloud/contacts/commit/ed0e80e91fc16c5d8133c4474d10a33e31d7089b))
* Improve team details ([870efdb](https://github.com/nextcloud/contacts/commit/870efdb68befa9dd11f3c397c8493d8fe1efa9e6))
* migrate app to Vue 3 ([6f3deff](https://github.com/nextcloud/contacts/commit/6f3deff747a307745d620646683724d6a1ee1389))
* **PropertyGroups:** sort groups alphabetically ([0ab7fea](https://github.com/nextcloud/contacts/commit/0ab7feab20ba46a8af2cc257ee0054d164edf396))
* Rebrand circles to teams in the frontend ([45b7dca](https://github.com/nextcloud/contacts/commit/45b7dcadcf61d28f52c8252164f6f7b991eeb8d7))
* rename and delete groups ([88d9e72](https://github.com/nextcloud/contacts/commit/88d9e72ed1b7664cdc7965abcb4a4209a34f62e6))
* **rfcProps:** make notes a default prop ([280dd19](https://github.com/nextcloud/contacts/commit/280dd19168c5d80891c92cac855fe9884c356604))
* **RootNavigation:** fill active icons ([dde3215](https://github.com/nextcloud/contacts/commit/dde321589e9fb2488bdaa9cbce13f9326d686539))
* show a user's groups as teams ([16b6be5](https://github.com/nextcloud/contacts/commit/16b6be57269a206cb3fdeaa90cea83cd2f131934))
* show loading icon when deleting or renaming groups ([0f43086](https://github.com/nextcloud/contacts/commit/0f43086c251a3a0986e50146d7398745bc999d7e))
* sort addressbooks ([3330dff](https://github.com/nextcloud/contacts/commit/3330dffc63dc467c93399c887570c2d2a8597a89))
* **TeamMemberList:** Merge indivual and team members into grid ([7e6109e](https://github.com/nextcloud/contacts/commit/7e6109e1628a8aca09804ebffb5faf8653473080))
* **teams:** add confirmation popup when enabling federation for a team ([ae073ee](https://github.com/nextcloud/contacts/commit/ae073ee2da5bfaa2cca5119b421da66a7b9d806d))
* **teams:** add swtich to enable/disable federation on team settings ([3fa979c](https://github.com/nextcloud/contacts/commit/3fa979cfb2aae36bdd4e91d55619ac374627500a))
* **teams:** Implement quick create for team resources ([81ef1b4](https://github.com/nextcloud/contacts/commit/81ef1b426bfa94b9dc69189be272930272d93762))
* **teams:** Member roles in list & responsive ui ([a83142e](https://github.com/nextcloud/contacts/commit/a83142ed5c3a514bf727ba665a63ab8265eb5c09))
* validate contact email ([c19fac5](https://github.com/nextcloud/contacts/commit/c19fac5277bf444ce127b70fbad80f7f255dc3eb))


### Performance Improvements

* migrate to vite ([5a2d57d](https://github.com/nextcloud/contacts/commit/5a2d57de2c265d29d1178e9aa828c5ed0d21e6d6))


### Reverts

* Revert "chore(deps-dev): bump vite from 5.4.19 to 6.3.4" ([05557dd](https://github.com/nextcloud/contacts/commit/05557ddc7927c2cfec69b189f1e50232cdb3e6cd))
* Revert "style(ContactsListItem): fix subtitle" ([1c1b865](https://github.com/nextcloud/contacts/commit/1c1b865272d27c0db23aed779a778b3c70ebba73))



# [5.3.0-beta2](https://github.com/nextcloud/contacts/compare/v5.3.0-beta1...v5.3.0-beta2) (2023-05-12)


### Bug Fixes

* **contacts:** display the new avatar on change ([de726e1](https://github.com/nextcloud/contacts/commit/de726e10d6a68907397060627fe1bcd53dca5cb8))



# [5.3.0-beta1](https://github.com/nextcloud/contacts/compare/v5.3.0-alpha1...v5.3.0-beta1) (2023-05-05)


### Bug Fixes

* **contacts:** defer birthday exclusion until contact was loaded ([2e1e139](https://github.com/nextcloud/contacts/commit/2e1e13980542bf53d6178d54ce9d9d23dda7fba2))
* **contacts:** display name not uri for recent-contacted address book ([6421bd4](https://github.com/nextcloud/contacts/commit/6421bd4499a90bf35ce777641a411f3ee64e43af))
* **contacts:** remove obsolete warning and fix header alignment ([8ef7e85](https://github.com/nextcloud/contacts/commit/8ef7e854a5f47dc4312e46c1f6446f74a2050866))
* defer rendering contact import button ([8435aa7](https://github.com/nextcloud/contacts/commit/8435aa7a9efaeec1312fe156eabbcca533c92cbf))
* disable select component if single option ([836bb9a](https://github.com/nextcloud/contacts/commit/836bb9af1733368baecaf21e42a7e28e40a06301))
* double scrollbar in details pane ([1f2f0fe](https://github.com/nextcloud/contacts/commit/1f2f0fef17c6312c03eb1da74496b0c8d26302a3))
* ellipsized "other social media" dropdown ([85fde3a](https://github.com/nextcloud/contacts/commit/85fde3a480e7356a84ec819d5a03d74267bf3361))
* improve design add new Property and change behaviour ([16b93f8](https://github.com/nextcloud/contacts/commit/16b93f8cee1ca48aa8301e70662de9d5587682f3))
* mime case matching ([fdbd984](https://github.com/nextcloud/contacts/commit/fdbd9849d42aa4441f1374dabff75c143c68dfe4))


### Features

* **2464:** exclude contact from birthday calendar ([e9730d6](https://github.com/nextcloud/contacts/commit/e9730d6b425cf30a6345685af9f987b166d4a16c))
* **contacts:** add add-prop button direct in prop ([9fa3e81](https://github.com/nextcloud/contacts/commit/9fa3e817cd7690b9c7e84a4c5ec61ba17fc7e9cd))
* **contacts:** adjust header to single column layout ([f348b2e](https://github.com/nextcloud/contacts/commit/f348b2e19c949a916e7b8974ea32e5d5c4add435))
* **contacts:** implement final design enhancements from mockups ([923d14b](https://github.com/nextcloud/contacts/commit/923d14b652e121b260ebad5cf1dfb9094ea257ba))
* **contacts:** implement read-only and edit modes ([0972b74](https://github.com/nextcloud/contacts/commit/0972b744116bfa7bd8fcb4ee9dd29c0eda23b872))
* **contacts:** implement single column layout ([7a7a95e](https://github.com/nextcloud/contacts/commit/7a7a95e4e428f33a7cd4969961832b786806ee42))
* **deps:** Add Nextcloud 27 support ([dc5ae56](https://github.com/nextcloud/contacts/commit/dc5ae56ce9977c54267788bbf32961f54a4d1ce2))
* **deps:** Revive PHP7.4 support ([508c5a1](https://github.com/nextcloud/contacts/commit/508c5a151d6b8f8c4443c38c151aa5dc566d2e72))


### Performance Improvements

* **autoloader:** Use Composer's authoritative classmap ([3ed7e01](https://github.com/nextcloud/contacts/commit/3ed7e017dbd72ebf6f3171fecfb489043945ff78))
* **bundles:** migrate nc-vue imports ([69a29d9](https://github.com/nextcloud/contacts/commit/69a29d9203382d0d86cd13e4ec03fc5cf79b205c))


### Reverts

* Revert "Bump vue and vue-template-compiler" ([75518ea](https://github.com/nextcloud/contacts/commit/75518ea21314884c4d8088f5f46f93410b36d8ba))
* Revert "Add manually-triggered branch-off workflow" ([d1f2ea1](https://github.com/nextcloud/contacts/commit/d1f2ea1925678f8629dc0476139ca92c977f214c))



# [5.0.0-alpha4](https://github.com/nextcloud/contacts/compare/v5.0.0-alpha3...v5.0.0-alpha4) (2022-09-27)



# [5.0.0-alpha3](https://github.com/nextcloud/contacts/compare/v5.0.0-alpha2...v5.0.0-alpha3) (2022-09-22)



# [5.0.0-alpha2](https://github.com/nextcloud/contacts/compare/v5.0.0-alpha1...v5.0.0-alpha2) (2022-09-19)



# [5.0.0-alpha1](https://github.com/nextcloud/contacts/compare/v4.1.0...v5.0.0-alpha1) (2022-09-16)



# [4.1.0](https://github.com/nextcloud/contacts/compare/v4.0.1...v4.1.0) (2022-03-22)



## [4.0.2](https://github.com/nextcloud/contacts/compare/v4.0.0...v4.0.2) (2021-08-25)



# [4.0.0-rc.0](https://github.com/nextcloud/contacts/compare/v4.0.0-beta.3...v4.0.0-rc.0) (2021-07-05)



# [4.0.0-beta.3](https://github.com/nextcloud/contacts/compare/v4.0.0-beta.2...v4.0.0-beta.3) (2021-06-21)



# [4.0.0-beta.2](https://github.com/nextcloud/contacts/compare/v3.5.1...v4.0.0-beta.2) (2021-06-17)



## [3.5.1](https://github.com/nextcloud/contacts/compare/v3.5.0...v3.5.1) (2021-03-17)



# [3.5.0](https://github.com/nextcloud/contacts/compare/v3.4.3...v3.5.0) (2021-03-15)



## [3.4.3](https://github.com/nextcloud/contacts/compare/v3.4.2...v3.4.3) (2021-01-04)



## [3.4.2](https://github.com/nextcloud/contacts/compare/v3.4.1...v3.4.2) (2020-11-18)



## [3.4.1](https://github.com/nextcloud/contacts/compare/v3.4.0...v3.4.1) (2020-10-20)



# [3.4.0](https://github.com/nextcloud/contacts/compare/v3.3.0...v3.4.0) (2020-09-30)



# [3.2.0](https://github.com/nextcloud/contacts/compare/v3.1.9...v3.2.0) (2020-02-29)



## [3.1.9](https://github.com/nextcloud/contacts/compare/v3.1.6...v3.1.9) (2020-02-26)



## [3.1.6](https://github.com/nextcloud/contacts/compare/v3.1.5...v3.1.6) (2019-10-03)



## [3.1.5](https://github.com/nextcloud/contacts/compare/v3.1.4...v3.1.5) (2019-10-03)



## [3.1.4](https://github.com/nextcloud/contacts/compare/v3.1.3...v3.1.4) (2019-09-28)



## [3.1.3](https://github.com/nextcloud/contacts/compare/v3.1.2...v3.1.3) (2019-06-11)



## [3.1.2](https://github.com/nextcloud/contacts/compare/v3.1.1...v3.1.2) (2019-06-10)



## [3.1.1](https://github.com/nextcloud/contacts/compare/v3.1.0...v3.1.1) (2019-04-16)



# [3.1.0](https://github.com/nextcloud/contacts/compare/v3.0.1...v3.1.0) (2019-04-08)



## [3.0.1](https://github.com/nextcloud/contacts/compare/v3.0.0...v3.0.1) (2018-12-24)



# [3.0.0-beta1](https://github.com/nextcloud/contacts/compare/v3.0.0-alpha1...v3.0.0-beta1) (2018-11-14)



## [2.1.6-beta](https://github.com/nextcloud/contacts/compare/v2.1.6...v2.1.6-beta) (2018-08-02)



## [2.1.5](https://github.com/nextcloud/contacts/compare/v2.1.4...v2.1.5) (2018-06-01)


### Reverts

* Revert "Init addressbook disabling ability" ([3455b79](https://github.com/nextcloud/contacts/commit/3455b790b877ac746f974cafb60c6f7f371f13ad))
* Revert "Do not rely on the file type but simply try to parse the given file as vcard - fixes #385" ([b28ef41](https://github.com/nextcloud/contacts/commit/b28ef41c9e47d7c791b29bc8c185067eba359718)), closes [#385](https://github.com/nextcloud/contacts/issues/385)
* Revert "Add support for X-ABLABEL refs #204" ([a4fe8a0](https://github.com/nextcloud/contacts/commit/a4fe8a06a814a216a20e8c861ada446f45a7ffbc)), closes [#204](https://github.com/nextcloud/contacts/issues/204)



